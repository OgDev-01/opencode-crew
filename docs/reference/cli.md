...
Complete reference for the `opencode-crew` command-line interface.
...
Interactive installation tool for initial OpenCode-Crew setup. Provides a TUI based on `@clack/prompts`.
...
3. **Configuration File Creation**: Generates `opencode.json` or `opencode-crew.json` files
4. **Plugin Registration**: Automatically registers the opencode-crew plugin in OpenCode settings
...
Diagnoses your environment to ensure OpenCode-Crew is functioning correctly. Performs 17+ health checks.
...
1. **Project Level**: `.opencode/opencode-crew.json`
2. **User Level**: `~/.config/opencode/opencode-crew.json`
