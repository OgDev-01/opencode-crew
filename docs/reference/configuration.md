...
Complete reference for `opencode-crew.jsonc` configuration. This document covers every available option with examples.
...
1. `.opencode/opencode-crew.jsonc` / `.opencode/opencode-crew.json`
2. User config (`.jsonc` preferred over `.json`):

| Platform | Path |
|----------|------|
| macOS/Linux | `~/.config/opencode/opencode-crew.jsonc` |
| Windows | `%APPDATA%\opencode\opencode-crew.jsonc` |
...
Override built-in agent settings. Available agents: `captain`, `craftsman`, `strategist`, `sage`, `archivist`, `lookout`, `spotter`, `assessor`, `critic`, `relay`.

```json
{
  "agents": {
    "lookout": { "model": "anthropic/claude-haiku-4-5", "temperature": 0.5 },
    "spotter": { "disable": true }
  }
}
```

Disable agents entirely: `{ "disabled_agents": ["sage", "spotter"] }`
...
{
  "agents": {
    "sage": { "thinking": { "type": "enabled", "budgetTokens": 200000 } }
  }
}
```
...
Domain-specific model delegation used by the `task()` tool. When Captain delegates work, it picks a category, not a model name.
...
|-------|---------------|-------------------|
| **Captain** | `claude-opus-4-6` | anthropic → github-copilot → opencode → kimi-for-coding → zai-coding-plan |
| **Craftsman** | `gpt-5.3-codex` | openai → github-copilot → opencode |
| **sage** | `gpt-5.2` | openai → google → anthropic (via github-copilot/opencode) |
| **archivist** | `glm-4.7` | zai-coding-plan → opencode → anthropic |
| **lookout** | `grok-code-fast-1` | github-copilot → anthropic/opencode → opencode |
| **spotter** | `gemini-3-flash` | google → openai → zai-coding-plan → kimi-for-coding → opencode → anthropic |
| **Strategist** | `claude-opus-4-6` | anthropic → kimi-for-coding → opencode → openai → google |
| **Assessor** | `claude-opus-4-6` | anthropic → kimi-for-coding → opencode → openai → google |
| **Critic** | `gpt-5.2` | openai → anthropic → google (via github-copilot/opencode) |
| **Relay** | `k2p5` | kimi-for-coding → opencode → anthropic → openai → google |
...
| Option | Default | Description |
|--------|---------|-------------|
| `disabled` | `false` | Disable all Captain orchestration, restore original build/plan |
| `default_builder_enabled` | `false` | Enable OpenCode-Builder agent (off by default) |
| `planner_enabled` | `true` | Enable Strategist (Planner) agent |
| `replace_plan` | `true` | Demote default plan agent to subagent mode |

Captain agents can also be customized under `agents` using their names: `Captain`, `OpenCode-Builder`, `Strategist (Planner)`, `Assessor (Plan Consultant)`.

### Captain Tasks

Enable the Captain Tasks system for cross-session task tracking.
...
| Option | Default | Description |
|--------|---------|-------------|
| `enabled` | `false` | Enable Captain Tasks system |
| `storage_path` | `.captain/tasks` | Storage path (relative to project root) |
...
Available hooks: `todo-continuation-enforcer`, `context-window-monitor`, `session-recovery`, `session-notification`, `comment-checker`, `grep-output-truncator`, `tool-output-truncator`, `directory-agents-injector`, `directory-readme-injector`, `empty-task-response-detector`, `think-mode`, `anthropic-context-window-limit-recovery`, `rules-injector`, `background-notification`, `auto-update-checker`, `startup-toast`, `keyword-detector`, `agent-usage-reminder`, `non-interactive-env`, `interactive-bash-session`, `compaction-context-injector`, `thinking-block-validator`, `claude-code-hooks`, `ralph-loop`, `preemptive-compaction`, `auto-slash-command`, `cadet-notepad`, `no-captain-gpt`, `start-work`, `runtime-fallback`

**Notes:**
- `directory-agents-injector` — auto-disabled on OpenCode 1.1.37+ (native AGENTS.md support)
- `no-captain-gpt` — **do not disable**. Captain is not optimized for GPT; this hook switches to Craftsman automatically.
...
| `disable_crew_env` | `false` | Disable auto-injected `<crew-env>` block (date/time/locale). Improves cache hit rate. |
| `task_system` | `false` | Enable Captain task system |
...
{
  "agents": {
    "lookout": { "model": "ollama/qwen3-coder", "stream": false }
  }
}
```
