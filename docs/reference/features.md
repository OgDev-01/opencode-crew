# OpenCode-Crew Features Reference

## Agents

OpenCode-Crew provides 11 specialized AI agents. Each has distinct expertise, optimized models, and tool permissions.

### Core Agents

| Agent | Model | Purpose |
|-------|-------|---------|
| **Captain** | `claude-opus-4-6` | The default orchestrator. Plans, delegates, and executes complex tasks using specialized subagents with aggressive parallel execution. Todo-driven workflow with extended thinking (32k budget). Fallback: gpt-5.3-codex → deep quality chain. |
| **Craftsman** | `gpt-5.3-codex` | The Legitimate Craftsman. Autonomous deep worker inspired by AmpCode's deep mode. Goal-oriented execution with thorough research before action. Lookouts codebase patterns, completes tasks end-to-end without premature stopping. Named after the Greek god of forge and craftsmanship. Fallback: deep quality chain (claude-opus-4-6-thinking → step-3.5-flash → glm-5 → ...). Requires at least one model in the chain to be available. |
| **Sage** | `gpt-5.3-codex` | Architecture decisions, code review, debugging. Read-only consultation with stellar logical reasoning and deep analysis. Inspired by AmpCode. Fallback: claude-opus-4-6-thinking → claude-sonnet-4-5-thinking → deep quality chain. |
| **Archivist** | `claude-sonnet-4-5` | Multi-repo analysis, documentation lookup, OSS implementation examples. Deep codebase understanding with evidence-based answers. Fallback: speed chain (claude-haiku-4-5 → gpt-5-mini → ...) → quality chain. |
| **Lookout** | `claude-haiku-4-5` | Fast codebase exploration and contextual grep. Fallback: oswe-vscode-prime → gpt-5-mini → gpt-4.1 → extended speed chain. |
...
| Agent | Model | Purpose |
|-------|-------|---------|
| **Strategist** | `claude-opus-4-6-thinking` | Strategic planner with interview mode. Creates detailed work plans through iterative questioning. Fallback: gpt-5.3-codex → claude-sonnet-4-5-thinking → deep quality chain. |
| **Assessor** | `claude-opus-4-6-thinking` | Plan consultant — pre-planning analysis. Identifies hidden intentions, ambiguities, and AI failure points. Fallback: gpt-5.3-codex → claude-sonnet-4-5-thinking → deep quality chain. |
| **Critic** | `gpt-5.3-codex` | Plan reviewer — validates plans against clarity, verifiability, and completeness standards. Fallback: claude-opus-4-6-thinking → deep quality chain. |

### Orchestration Agents

| Agent | Model | Purpose |
|-------|-------|---------|
| **Relay** | `claude-sonnet-4-5-thinking` | Todo-list orchestrator. Executes planned tasks systematically, managing todo items and coordinating work. Fallback: claude-opus-4-6-thinking → gpt-5.3-codex → deep quality chain. |
| **Cadet** | *(category-dependent)* | Category-spawned executor. Model is selected automatically based on the task category (visual-engineering, quick, deep, etc.). Used when the main agent delegates work via the `task` tool. |
...
```
Ask @sage to review this design and propose an architecture
Ask @archivist how this is implemented - why does the behavior keep changing?
Ask @lookout for the policy on this feature
```
...
| Agent | Restrictions |
|-------|-------------|
| sage | Read-only: cannot write, edit, or delegate (blocked: write, edit, task, call_agent) |
| archivist | Cannot write, edit, or delegate (blocked: write, edit, task, call_agent) |
| lookout | Cannot write, edit, or delegate (blocked: write, edit, task, call_agent) |
| spotter | Allowlist: `read` only |
| relay | Cannot delegate (blocked: task, call_agent) |
...
task(subagent_type="lookout", load_skills=[], prompt="Find auth implementations", run_in_background=true)
...
Customize agent models, prompts, and permissions in `opencode-crew.json`.
...
You can define custom categories in `opencode-crew.json`.
...
### Cadet as Delegated Executor

When you use a Category, a special agent called **Cadet** performs the work.
...
OpenCode-Crew provides two browser automation providers, configurable via `browser_automation_engine.provider`.
...
| `/start-work` | Start Captain work session from Strategist plan |
...
**Purpose**: Start execution from a Strategist-generated plan
...
Uses relay agent to execute planned tasks systematically.
...
| Tool | Description |
|------|-------------|
| **call_agent** | Spawn lookout/archivist agents. Supports `run_in_background`. |
...
**Storage**: Tasks are stored as JSON files in `.captain/tasks/`.
...
| Hook | Event | Description |
|------|-------|-------------|
| **auto-update-checker** | Event | Checks for new versions on session creation, shows startup toast with version and Captain status. |
...
| **relay** | Multiple | Main orchestration logic for todo-driven work sessions. |
...
| Hook | Event | Description |
|------|-------|-------------|
| **strategist-md-only** | PreToolUse | Enforces markdown-only output for Strategist planner. |
| **no-captain-gpt** | Message | Prevents Captain from running on incompatible GPT models. |
| **no-craftsman-non-gpt** | Message | Prevents Craftsman from running on non-GPT models. |
| **cadet-notepad** | PreToolUse | Manages notepad state for Cadet agents. |
