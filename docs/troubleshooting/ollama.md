...
When using Ollama as a provider with opencode-crew agents, you may encounter:

```
JSON Parse error: Unexpected EOF
```

This occurs when agents attempt tool calls (e.g., `lookout` agent using `mcp_grep_search`).
...
- **opencode-crew**: Passes through the SDK's behavior (can't fix at this layer)
...
- **Problematic**: Any agent with tool calls (lookout, archivist, etc.)
...
**Tracking**: https://github.com/buldtech/opencode-crew/issues/1124
...
- **opencode-crew**: https://github.com/buldtech/opencode-crew/issues/1124
