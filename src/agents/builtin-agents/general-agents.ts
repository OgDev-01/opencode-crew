...
export function collectPendingBuiltinAgents(input: {
  ...
  for (const [name, source] of Object.entries(agentSources)) {
    ...
    if (agentName === "captain") continue
    if (agentName === "craftsman") continue
    if (agentName === "relay") continue
    ...
    if (agentName === "archivist") {
      config = applyEnvironmentContext(config, directory, { disableCrewEnv })
    }

    config = applyOverrides(config, override, mergedCategories, directory)

    // Store for later - will be added after captain and craftsman
