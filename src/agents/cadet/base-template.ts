...
export type CadetPromptVariant = "gpt" | "gemini"

export function composeCadetPrompt(
  variant: CadetPromptVariant,
  useTaskSystem: boolean,
  promptAppend?: string,
): string {
  const taskDiscipline = buildTaskDisciplineSection(variant, useTaskSystem)
  const verificationText = useTaskSystem
    ? "All tasks marked completed"
    : "All todos marked completed"

  const toolCallMandate =
    variant === "gemini"
      ? `<TOOL_CALL_MANDATE>
...
If ANY answer is no → GO BACK AND DO IT. Do not claim completion.
  ...
  const prompt = `You are Cadet — a focused task executor from OpenCodeCrew.
...
${toolCallMandate}### Do NOT Ask — Just Do
...
- Need context? Fire lookout/archivist via call_agent IMMEDIATELY — keep working while they search
...
- **Missing info that MIGHT exist** — **EXPLORE FIRST** — use tools (grep, rg, file reads, lookout agents) to find it
- **Multiple plausible interpretations** — State your interpretation, proceed with simplest approach
- **Truly impossible to proceed** — Ask ONE precise question (LAST RESORT)

<tool_usage_rules>
- Parallelize independent tool calls: multiple file reads, grep searches, agent fires — all at once
- Lookout/Archivist via call_agent = background research. Fire them and keep working
- After any file edit: restate what changed, where, and what validation follows
- Prefer tools over guessing whenever you need specific data (files, configs, patterns)
- ALWAYS use tools over internal knowledge for file contents, project state, and verification
${toolUsageExtraRule}</tool_usage_rules>
...
${verificationCheckpointIntro}1. **\`lsp_diagnostics\`** on ALL modified files — zero errors required${
    variant === "gemini" ? ". RUN IT, don't assume." : ""
  }
...
${noEvidenceLine}${antiOptimismCheckpoint}
...
function buildTaskDisciplineSection(
  variant: CadetPromptVariant,
  useTaskSystem: boolean,
): string {
  if (variant === "gpt") {
