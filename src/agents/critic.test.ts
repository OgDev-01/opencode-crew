...
import { CRITIC_SYSTEM_PROMPT } from "./critic"

function escapeRegExp(value: string) {
  return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
}

describe("CRITIC_SYSTEM_PROMPT policy requirements", () => {
  ...
  test("should extract paths containing .crew/plans/ and ending in .md", () => {
    // given
    const prompt = CRITIC_SYSTEM_PROMPT

    // when / #then
    expect(prompt).toContain(".crew/plans/")
  ...
  test("should NOT teach that 'Please review' is INVALID (conversational wrapper allowed)", () => {
    ...
    const invalidExample = "Please review .crew/plans/plan.md"
