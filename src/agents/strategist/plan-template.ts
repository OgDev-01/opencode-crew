...
Generate plan to: \`.crew/plans/{name}.md\`
...
- [Explicit exclusion from Assessor review]
...
Evidence saved to \`.crew/evidence/task-{N}-{scenario-slug}.{ext}\`.
...
├── Task F1: Plan compliance audit (sage)
...
- **FINAL**: **4** — F1 → \`sage\`, F2 → \`unspecified-high\`, F3 → \`unspecified-high\`, F4 → \`deep\`
...
- [ ] 1. [Task Title]
  ...
  **QA Scenarios (MANDATORY — task is INCOMPLETE without these):**
  ...
  Scenario: [Happy path — what SHOULD work]
    ...
    Evidence: .crew/evidence/task-{N}-{scenario-slug}.{ext}
  ...
  Scenario: [Failure/edge case — what SHOULD fail gracefully]
    ...
    Evidence: .crew/evidence/task-{N}-{scenario-slug}-error.{ext}
  \\\`\\\`\\\`
...
- [ ] F1. **Plan Compliance Audit** — \`sage\`
  Read the plan end-to-end. For each "Must Have": verify implementation exists (read file, curl endpoint, run command). For each "Must NOT Have": search codebase for forbidden patterns — reject with file:line if found. Check evidence files exist in .crew/evidence/. Compare deliverables against plan.
...
- [ ] F3. **Real Manual QA** — \`unspecified-high\` (+ \`playwright\` skill if UI)
  Start from clean state. Execute EVERY QA scenario from EVERY task — follow exact steps, capture evidence. Test cross-task integration (features working together, not isolation). Test edge cases: empty state, invalid input, rapid actions. Save to \`.crew/evidence/final-qa/\`.
