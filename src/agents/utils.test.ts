...
import * as connectedProvidersCache from "../shared/model/connected-providers-cache"
import * as modelAvailability from "../shared/model/model-availability"
import * as shared from "../shared"

const TEST_DEFAULT_MODEL = "anthropic/claude-opus-4-6"

describe("createBuiltinAgents with model overrides", () => {
  test("Captain with default model has thinking config when all models available", async () => {
    // #given
    ...
    try {
      ...
      // #then
      expect(agents.captain.model).toBe("anthropic/claude-opus-4-6")
      expect(agents.captain.thinking).toEqual({ type: "enabled", budgetTokens: 32000 })
      expect(agents.captain.reasoningEffort).toBeUndefined()
    } finally {
      fetchSpy.mockRestore()
    }
  })

  test("Captain with GPT model override has reasoningEffort, no thinking", async () => {
    // #given
    const overrides = {
      captain: { model: "github-copilot/gpt-5.2" },
    }
    ...
    // #then
    expect(agents.captain.model).toBe("github-copilot/gpt-5.2")
    expect(agents.captain.reasoningEffort).toBe("medium")
    expect(agents.captain.thinking).toBeUndefined()
  })

  test("Relay uses uiSelectedModel", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
    ...
    try {
      ...
      // #then
      expect(agents.relay).toBeDefined()
      expect(agents.relay.model).toBe("openai/gpt-5.2")
    } finally {
      fetchSpy.mockRestore()
    }
  })

  test("user config model takes priority over uiSelectedModel for captain", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
      new Set(["openai/gpt-5.2", "anthropic/claude-sonnet-4-6"])
    )
    const uiSelectedModel = "openai/gpt-5.2"
    const overrides = {
      captain: { model: "google/antigravity-claude-opus-4-5-thinking" },
    }
    ...
    try {
      ...
      // #then
      expect(agents.captain).toBeDefined()
      expect(agents.captain.model).toBe("google/antigravity-claude-opus-4-5-thinking")
    } finally {
      fetchSpy.mockRestore()
    }
  })

  test("user config model takes priority over uiSelectedModel for relay", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
      new Set(["openai/gpt-5.2", "anthropic/claude-sonnet-4-6"])
    )
    const uiSelectedModel = "openai/gpt-5.2"
    const overrides = {
      relay: { model: "google/antigravity-claude-opus-4-5-thinking" },
    }
    ...
    try {
      ...
      // #then
      expect(agents.relay).toBeDefined()
      expect(agents.relay.model).toBe("google/antigravity-claude-opus-4-5-thinking")
    } finally {
      fetchSpy.mockRestore()
    }
  })

  test("Captain is created on first run when no availableModels or cache exist", async () => {
    // #given
   ...
   test("Sage uses connected provider fallback when availableModels is empty and cache exists", async () => {
     // #given - connected providers cache has "openai", which matches sage's first fallback entry
     ...
     // #then - sage resolves via connected cache fallback to openai/gpt-5.2 (not system default)
     expect(agents.sage.model).toBe("openai/gpt-5.2")
     expect(agents.sage.reasoningEffort).toBe("medium")
     expect(agents.sage.thinking).toBeUndefined()
     cacheSpy.mockRestore?.()
   })

   test("Sage created without model field when no cache exists (first run scenario)", async () => {
     ...
     // #then - sage should be created with system default model (fallback to systemDefaultModel)
     expect(agents.sage).toBeDefined()
     expect(agents.sage.model).toBe(TEST_DEFAULT_MODEL)
     cacheSpy.mockRestore?.()
   })

  test("Sage with GPT model override has reasoningEffort, no thinking", async () => {
    // #given
    const overrides = {
    ...
    expect(agents.sage.model).toBe("openai/gpt-5.2")
    expect(agents.sage.reasoningEffort).toBe("medium")
    expect(agents.sage.textVerbosity).toBe("high")
    expect(agents.sage.thinking).toBeUndefined()
  })

  test("Sage with Claude model override has thinking, no reasoningEffort", async () => {
    // #given
    const overrides = {
      sage: { model: "anthropic/claude-sonnet-4" },
    }
    ...
    // #then
    expect(agents.sage.model).toBe("anthropic/claude-sonnet-4")
    expect(agents.sage.thinking).toEqual({ type: "enabled", budgetTokens: 32000 })
    expect(agents.sage.reasoningEffort).toBeUndefined()
    expect(agents.sage.textVerbosity).toBeUndefined()
  })

   test("non-model overrides are still applied after factory rebuild", async () => {
     // #given
     const overrides = {
       captain: { model: "github-copilot/gpt-5.2", temperature: 0.5 },
     }
     ...
     // #then
     expect(agents.captain.model).toBe("github-copilot/gpt-5.2")
     expect(agents.captain.temperature).toBe(0.5)
   })
  ...
  test("createBuiltinAgents excludes disabled skills from availableSkills", async () => {
    ...
    // #then
    expect(agents.captain.prompt).not.toContain("playwright")
    expect(agents.captain.prompt).toContain("frontend-ui-ux")
    expect(agents.captain.prompt).toContain("git-master")
  })
  ...
  test("includes custom agents in orchestrator prompts when provided via config", async () => {
    ...
    try {
      ...
      // #then
      expect(agents.captain.prompt).toContain("researcher")
      expect(agents.craftsman.prompt).toContain("researcher")
      expect(agents.relay.prompt).toContain("researcher")
    } finally {
      fetchSpy.mockRestore()
    }
  })
  ...
  test("excludes hidden custom agents from orchestrator prompts", async () => {
    ...
    try {
      ...
      // #then
      expect(agents.captain.prompt).not.toContain("hidden-agent")
      expect(agents.craftsman.prompt).not.toContain("hidden-agent")
      expect(agents.relay.prompt).not.toContain("hidden-agent")
    } finally {
      fetchSpy.mockRestore()
    }
  })
  ...
  test("excludes disabled custom agents from orchestrator prompts", async () => {
    ...
    try {
      ...
      // #then
      expect(agents.captain.prompt).not.toContain("disabled-agent")
      expect(agents.craftsman.prompt).not.toContain("disabled-agent")
      expect(agents.relay.prompt).not.toContain("disabled-agent")
    } finally {
      fetchSpy.mockRestore()
    }
  })
  ...
  test("excludes custom agents when disabledAgents contains their name (case-insensitive)", async () => {
    ...
    try {
      ...
      // #then
      expect(agents.captain.prompt).not.toContain("researcher")
      expect(agents.craftsman.prompt).not.toContain("researcher")
      expect(agents.relay.prompt).not.toContain("researcher")
    } finally {
      fetchSpy.mockRestore()
    }
  })
  ...
  test("deduplicates custom agents case-insensitively", async () => {
    ...
    try {
      ...
      // #then
      const matches = (agents.captain?.prompt ?? "").match(/Custom agent: researcher/gi) ?? []
  ...
  test("sanitizes custom agent strings for markdown tables", async () => {
    ...
    try {
      ...
      // #then
      expect(agents.captain.prompt).toContain("Line1 Alpha \\| Beta")
    } finally {
      fetchSpy.mockRestore()
    }
  })
...
describe("createBuiltinAgents without systemDefaultModel", () => {
   test("agents created via connected cache fallback even without systemDefaultModel", async () => {
     // #given - connected cache has "openai", which matches sage's fallback chain
     const cacheSpy = spyOn(connectedProvidersCache, "readConnectedProvidersCache").mockReturnValue(["openai"])
   ...
   test("agents NOT created when no cache and no systemDefaultModel (first run without defaults)", async () => {
     ...
     // #then
     expect(agents.sage).toBeUndefined()
     cacheSpy.mockRestore?.()
   })

  test("captain created via connected cache fallback when all providers available", async () => {
    // #given
...
describe("Craftsman environment context toggle", () => {
  ...
  test("includes <crew-env> tag when disable flag is unset", async () => {
    // #when
    const agents = await buildAgents(undefined)

    // #then
    expect(agents.craftsman).toBeDefined()
    expect(agents.craftsman.prompt).toContain("<crew-env>")
  })

  test("includes <crew-env> tag when disable flag is false", async () => {
    // #when
  ...
  test("omits <crew-env> tag when disable flag is true", async () => {
    // #when
    const agents = await buildAgents(true)

    // #then
    expect(agents.craftsman).toBeDefined()
    expect(agents.craftsman.prompt).not.toContain("<crew-env>")
  })
})

describe("Captain and Archivist environment context toggle", () => {
  let fetchSpy: ReturnType<typeof spyOn>
  ...
  test("includes <crew-env> for captain and archivist when disable flag is unset", async () => {
    const agents = await buildAgents(undefined)

    expect(agents.captain).toBeDefined()
    expect(agents.archivist).toBeDefined()
    expect(agents.captain.prompt).toContain("<crew-env>")
    expect(agents.archivist.prompt).toContain("<crew-env>")
  })

  test("includes <crew-env> for captain and archivist when disable flag is false", async () => {
    const agents = await buildAgents(false)
  ...
  test("omits <crew-env> for captain and archivist when disable flag is true", async () => {
    const agents = await buildAgents(true)

    expect(agents.captain).toBeDefined()
    expect(agents.archivist).toBeDefined()
    expect(agents.captain.prompt).not.toContain("<crew-env>")
    expect(agents.archivist.prompt).not.toContain("<crew-env>")
  })
})

describe("Relay is unaffected by environment context toggle", () => {
  let fetchSpy: ReturnType<typeof spyOn>
  ...
  test("relay prompt is unchanged and never contains <crew-env>", async () => {
    ...
    expect(agentsDefault.relay).toBeDefined()
    expect(agentsDisabled.relay).toBeDefined()
    expect(agentsDefault.relay.prompt).not.toContain("<crew-env>")
    expect(agentsDisabled.relay.prompt).not.toContain("<crew-env>")
    expect(agentsDisabled.relay.prompt).toBe(agentsDefault.relay.prompt)
  })
})

describe("createBuiltinAgents with requiresAnyModel gating (captain)", () => {
  test("captain is created when at least one fallback model is available", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
      new Set(["anthropic/claude-opus-4-6"])
    )
  ...
  test("captain is created when explicit config provided even if no models available", async () => {
    // #given
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(new Set())
    const overrides = {
  ...
  test("captain is not created when no fallback model is available and provider not connected", async () => {
    // #given - only openai/gpt-5.2 available, not in captain fallback chain
    const fetchSpy = spyOn(shared, "fetchAvailableModels").mockResolvedValue(
    ...
    try {
      ...
      // #then
      expect(agents.captain).toBeUndefined()
    } finally {
      fetchSpy.mockRestore()
      cacheSpy.mockRestore()
    }
  ...
  test("captain uses user-configured plugin model even when not in cache or fallback chain", async () => {
  ...
  test("captain uses user-configured plugin model when availableModels is empty but cache exists", async () => {
...
describe("override.category expansion in createBuiltinAgents", () => {
  test("standard agent override with category expands category properties", async () => {
    // #given
    const overrides = {
      sage: { category: "ultrabrain" } as any,
    }
    ...
    expect(agents.sage).toBeDefined()
    expect(agents.sage.model).toBe("openai/gpt-5.3-codex")
    expect(agents.sage.variant).toBe("xhigh")
  })
  ...
  test("standard agent override with category AND direct variant - direct wins", async () => {
    ...
    const overrides = {
      sage: { category: "ultrabrain", variant: "max" } as any,
    }
    ...
    expect(agents.sage).toBeDefined()
    expect(agents.sage.variant).toBe("max")
  })
  ...
  test("standard agent override with category AND direct reasoningEffort - direct wins", async () => {
    ...
    const overrides = {
      sage: { category: "test-cat", reasoningEffort: "low" } as any,
    }
    ...
    expect(agents.sage).toBeDefined()
    expect(agents.sage.reasoningEffort).toBe("low")
  })
  ...
  test("standard agent override with category applies reasoningEffort from category when no direct override", async () => {
    ...
    const overrides = {
      sage: { category: "reasoning-cat" } as any,
    }
    ...
    expect(agents.sage).toBeDefined()
    expect(agents.sage.reasoningEffort).toBe("high")
  })

  test("captain override with category expands category properties", async () => {
    // #given
    const overrides = {
      captain: { category: "ultrabrain" } as any,
    }
    ...
    // #then - ultrabrain category: model=openai/gpt-5.3-codex, variant=xhigh
    expect(agents.captain).toBeDefined()
    expect(agents.captain.model).toBe("openai/gpt-5.3-codex")
    expect(agents.captain.variant).toBe("xhigh")
  })

  test("relay override with category expands category properties", async () => {
    // #given
    const overrides = {
      relay: { category: "ultrabrain" } as any,
    }
    ...
    // #then - ultrabrain category: model=openai/gpt-5.3-codex, variant=xhigh
    expect(agents.relay).toBeDefined()
    expect(agents.relay.model).toBe("openai/gpt-5.3-codex")
    expect(agents.relay.variant).toBe("xhigh")
  })

  test("override with non-existent category has no effect on config", async () => {
    // #given
    const overrides = {
      sage: { category: "non-existent-category" } as any,
    }
    ...
    expect(agents.sage.model).toBe(agentsWithoutOverride.sage.model)
  })
})
...
describe("agent override tools migration", () => {
  test("tools: { x: false } is migrated to permission: { x: deny }", async () => {
    // #given
    const overrides = {
      lookout: { tools: { "jetbrains_*": false } } as any,
    }
    ...
    // #then
    expect(agents.lookout).toBeDefined()
    const permission = agents.lookout.permission as Record<string, string>
  ...
  test("tools: { x: true } is migrated to permission: { x: allow }", async () => {
    // #given
    const overrides = {
      archivist: { tools: { "jetbrains_get_*": true } } as any,
    }
    ...
    // #then
    expect(agents.archivist).toBeDefined()
    const permission = agents.archivist.permission as Record<string, string>
  ...
  test("tools config is removed after migration", async () => {
    // #given
    const overrides = {
      lookout: { tools: { "some_tool": false } } as any,
    }
    ...
    // #then
    expect(agents.lookout).toBeDefined()
    expect((agents.lookout as any).tools).toBeUndefined()
  })
})
