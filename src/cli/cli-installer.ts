...
export async function runCliInstaller(args: InstallArgs, version: string): Promise<number> {
  ...
  if (!validation.valid) {
    ...
    console.log()
    printInfo(
      "Usage: bunx opencode-crew install --no-tui --claude=<no|yes|max20> --gemini=<no|yes> --copilot=<no|yes>",
    )
    console.log()
    return 1
  }
  ...
  printStep(step++, totalSteps, "Writing opencode-crew configuration...")
  const omoResult = writeCrewConfig(config)
  ...
  if (!config.hasClaude) {
    ...
    console.log()
    console.log(color.red(color.bold("  Captain agent is STRONGLY optimized for Claude Opus 4.5.")))
