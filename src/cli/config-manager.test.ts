...
describe("getPluginNameWithVersion", () => {
  ...
  test("returns @latest when current version matches latest tag", async () => {
    ...
    expect(result).toBe("@buldtech/opencode-crew@latest")
  })
  ...
  test("returns @beta when current version matches beta tag", async () => {
    ...
    expect(result).toBe("@buldtech/opencode-crew@beta")
  })
  ...
  test("returns @next when current version matches next tag", async () => {
    ...
    expect(result).toBe("@buldtech/opencode-crew@next")
  })
  ...
  test("returns bare package name when npm returns non-ok response for stable version", async () => {
    ...
    expect(result).toBe("@buldtech/opencode-crew")
  })
...
describe("generateOmoConfig - model fallback system", () => {
  ...
  test("uses ZAI model for archivist when Z.ai is available", () => {
    ...
    // #then Captain uses Claude (OR logic)
    expect((result.agents as Record<string, { model: string }>).captain.model).toBe("anthropic/claude-opus-4-6")
  })
  ...
  test("uses native OpenAI models when only ChatGPT available", () => {
    ...
    // #then Captain is omitted (requires all fallback providers)
    expect((result.agents as Record<string, { model: string }>).captain).toBeUndefined()
    // #then Sage should use native OpenAI (first fallback entry)
    expect((result.agents as Record<string, { model: string }>).sage.model).toBe("openai/gpt-5.2")
    // #then spotter should use native OpenAI (fallback within native tier)
    expect((result.agents as Record<string, { model: string }>)["spotter"].model).toBe("openai/gpt-5.2")
  })

  test("uses haiku for lookout when Claude max20", () => {
  ...
  test("uses haiku for lookout regardless of max20 flag", () => {
