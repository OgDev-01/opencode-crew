...
| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `opencode-crew` in `.opencode/opencode.json` plugin array |
...
| `plugin-name-with-version.ts` | Resolve `opencode-crew@X.Y.Z` for installation |
| `write-omo-config.ts` | Write generated config to `.opencode/opencode-crew.jsonc` |
