...
describe("model-resolution check", () => {
  describe("getModelResolutionInfo", () => {
    ...
    it("returns agent requirements with provider chains", async () => {
      ...
      const captain = info.agents.find((a) => a.name === "captain")
      expect(captain).toBeDefined()
      expect(captain!.requirement.fallbackChain[0]?.model).toBe("claude-opus-4-6")
      expect(captain!.requirement.fallbackChain[0]?.providers).toContain("anthropic")
    })
  ...
  describe("getModelResolutionInfoWithOverrides", () => {
    // given: User has overrides in opencode-crew.json
    ...
    it("shows user override for agent when configured", async () => {
      ...
      // given: User has override for sage agent
      const mockConfig = {
        agents: {
      ...
      // then: Sage should show the override
      const sage = info.agents.find((a) => a.name === "sage")
      expect(sage).toBeDefined()
      expect(sage!.userOverride).toBe("anthropic/claude-opus-4-6")
      expect(sage!.effectiveResolution).toBe("User override: anthropic/claude-opus-4-6")
    })
    ...
    it("shows provider fallback when no override exists", async () => {
      ...
      const captain = info.agents.find((a) => a.name === "captain")
      expect(captain).toBeDefined()
      expect(captain!.userOverride).toBeUndefined()
      expect(captain!.effectiveResolution).toContain("Provider fallback:")
      expect(captain!.effectiveResolution).toContain("anthropic")
    })

    it("captures user variant for agent when configured", async () => {
      const { getModelResolutionInfoWithOverrides } = await import("./model-resolution")

      //#given User has model with variant override for sage agent
      const mockConfig = {
        agents: {
          sage: { model: "openai/gpt-5.2", variant: "xhigh" },
        },
      }
      ...
      //#then Sage should have userVariant set
      const sage = info.agents.find((a) => a.name === "sage")
      expect(sage).toBeDefined()
      expect(sage!.userOverride).toBe("openai/gpt-5.2")
      expect(sage!.userVariant).toBe("xhigh")
    })
