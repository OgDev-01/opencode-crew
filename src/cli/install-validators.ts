...
export function formatConfigSummary(config: InstallConfig): string {
  ...
  lines.push(formatProvider("OpenAI/ChatGPT", config.hasOpenAI, "GPT-5.2 for Sage"))
  lines.push(formatProvider("Gemini", config.hasGemini))
  lines.push(formatProvider("GitHub Copilot", config.hasCopilot, "fallback"))
  lines.push(formatProvider("OpenCode Zen", config.hasOpencodeZen, "opencode/ models"))
  lines.push(formatProvider("Z.ai Coding Plan", config.hasZaiCodingPlan, "Archivist/Multimodal"))
  lines.push(formatProvider("Kimi For Coding", config.hasKimiForCoding, "Captain/Strategist fallback"))
...
export function printHeader(isUpdate: boolean): void {
  ...
  console.log()
  console.log(color.bgMagenta(color.white(` CrewCrewCrewCrew... ${mode} `)))
  console.log()
}
