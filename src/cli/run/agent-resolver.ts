...
const CORE_AGENT_ORDER = ["captain", "craftsman", "strategist", "relay"] as const
const DEFAULT_AGENT = "captain"
...
const isAgentDisabled = (agentConfigKey: string, config: OpenCodeCrewConfig): boolean => {
  const lowered = agentConfigKey.toLowerCase()
  if (lowered === DEFAULT_AGENT && config.captain_agent?.disabled === true) {
    return true
  }
...
const pickFallbackAgent = (config: OpenCodeCrewConfig): CoreAgentKey => {
