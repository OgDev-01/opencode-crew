...
22 schema files composing `OpenCodeCrewConfigSchema`. Zod v4 validation with `safeParse()`. All fields optional — omitted fields use plugin defaults.

## SCHEMA TREE

```
config/schema/
├── opencode-crew-config.ts    # ROOT: OpenCodeCrewConfigSchema (composes all below)
...
├── captain.ts                 # CaptainConfigSchema (task system)
├── captain-agent.ts           # CaptainAgentConfigSchema
...
2. Add field to `opencode-crew-config.ts` root schema
