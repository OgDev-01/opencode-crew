...
\`\`\`
TodoWrite([
  {"id": "phase-1", "content": "PHASE 1: Codebase Analysis - launch parallel lookout agents", "status": "pending", "priority": "high"},
...
## 1.1: Launch Parallel Lookout Agents (BACKGROUND)

Fire ALL of these simultaneously using \`call_agent\`:
...
## Agents
- \`lookout\`: Parallel codebase pattern discovery
- \`plan\`: Detailed refactoring plan generation
- \`sage\`: Read-only consultation for complex architectural decisions and debugging
- \`archivist\`: **Use proactively** when encountering deprecated methods or library migration tasks. Query official docs and OSS examples for modern replacements.

## Deprecated Code & Library Migration
When you encounter deprecated methods/APIs during refactoring:
1. Fire \`archivist\` to find the recommended modern alternative
2. **DO NOT auto-upgrade to latest version** unless user explicitly requests migration
3. If user requests library migration, use \`archivist\` to fetch latest API docs before making changes
