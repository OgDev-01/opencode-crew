...
import { getOpenCodeConfigDir } from "@/shared/opencode/opencode-config-dir"
import type { z } from "zod"
import type { OpenCodeCrewConfig } from "@/config/schema"

export function getTaskDir(config: Partial<OpenCodeCrewConfig> = {}): string {
  const tasksConfig = config.captain?.tasks
...
export function resolveTaskListId(config: Partial<OpenCodeCrewConfig> = {}): string {
  ...
  const configId = config.captain?.tasks?.task_list_id?.trim()
...
export function listTaskFiles(config: Partial<OpenCodeCrewConfig> = {}): string[] {
