...
import { isSqliteBackend, resetSqliteBackendCache } from "../../shared/opencode/opencode-storage-detection"

//#region Mocks

const mockIsSqliteBackend = vi.fn()

vi.mock("../../shared/opencode/opencode-storage-detection", () => ({
...
describe("findNearestMessageWithFieldsFromSDK", () => {
  it("returns message with all fields when available", async () => {
    const mockClient = createMockClient([
      { info: { agent: "captain", model: { providerID: "anthropic", modelID: "claude-opus-4" } } },
    ])
  ...
  it("returns message with assistant shape (providerID/modelID directly on info)", async () => {
    const mockClient = createMockClient([
      { info: { agent: "captain", providerID: "openai", modelID: "gpt-5" } },
    ])
