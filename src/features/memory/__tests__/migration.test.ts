    ...
    describe("#when MemoryConfigSchema parses empty object", () => {
      describe("#then", () => {
        it("produces complete defaults with no undefined fields", () => {
          ...
          expect("embedding_model" in result).toBe(false)
          expect("ttl_golden_rules_days" in result).toBe(false)
          expect("ttl_heuristics_days" in result).toBe(false)
          expect("delegation_cost_awareness" in result).toBe(false)
