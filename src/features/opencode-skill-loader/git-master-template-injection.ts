...
export function injectGitMasterConfig(template: string, config?: GitMasterConfig): string {
	...
	sections.push("")
	sections.push("Add Captain attribution to EVERY commit:")
	sections.push("")
	...
	if (commitFooter) {
		const footerText =
			typeof commitFooter === "string"
				? commitFooter
				: "Ultraworked with [Captain](https://github.com/code-yeongyu/opencode-crew)"
	...
	if (includeCoAuthoredBy) {
		...
		sections.push("```")
		sections.push("Co-authored-by: Captain <clio-agent@opencodecrew.ai>")
		sections.push("```")
		sections.push("")
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(
			`git commit -m "{Commit Message}" -m "${footerText}" -m "Co-authored-by: Captain <clio-agent@opencodecrew.ai>"`
		)
		sections.push("```")
		...
		sections.push("```bash")
		sections.push(
			"git commit -m \"{Commit Message}\" -m \"Co-authored-by: Captain <clio-agent@opencodecrew.ai>\""
		)
		sections.push("```")
	}
