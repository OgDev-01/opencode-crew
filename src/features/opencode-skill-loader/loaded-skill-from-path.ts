...
import { sanitizeModelField } from "@/shared/model/model-sanitizer"
...
function createLazyLoader(skillPath: string, resolvedPath: string): LazyContentLoader {
  const loader: LazyContentLoader = {
    loaded: false,
    load: async () => {
      if (loader.loaded && loader.content !== undefined) {
...
export async function loadSkillFromPath(options: {
  ...
  try {
    ...
    return {
      ...
      mcpConfig,
      lazyContent: createLazyLoader(options.skillPath, options.resolvedPath),
    }
  } catch {
    return null
  }
