...
```
hooks/
├── relay/                      # Main orchestration (757 lines)
...
├── strategist-md-only/         # Planner read-only mode
...
├── start-work/                 # Captain work session starter
...
├── cadet-notepad/    # Captain Junior notepad
...
| strategistMdOnly | tool.execute.before | Enforce .md-only writes for Strategist |
| cadetNotepad | chat.message | Notepad injection for subagents |
...
| noCaptainGpt | chat.message | Block Captain from using GPT models (toast warning) |
| noCraftsmanNonGpt | chat.message | Block Craftsman from using non-GPT models |
...
| relayHook | event | Master orchestrator for boulder/background sessions |
...
Self-referential dev loop via `/ralph-loop` command. State persisted in `.crew/ralph-loop.local.md`. Detects `<promise>DONE</promise>` in AI output. Max 100 iterations default.
