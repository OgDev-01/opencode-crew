...
export function createAutoUpdateCheckerHook(ctx: PluginInput, options: AutoUpdateCheckerOptions = {}) {
  const { showStartupToast = true, isCaptainEnabled = false, autoUpdate = true } = options
  const isCliRunMode = process.env.OPENCODE_CLI_RUN_MODE === "true"

  const getToastMessage = (isUpdate: boolean, latestVersion?: string): string => {
    if (isCaptainEnabled) {
      return isUpdate
        ? `Captain on steroids is steering OpenCode.\nv${latestVersion} available. Restart to apply.`
        : "Captain on steroids is steering OpenCode."
    }
    return isUpdate
      ? `OpenCode is now on Steroids. CrewCrewCrewCrew...\nv${latestVersion} available. Restart OpenCode to apply.`
      : "OpenCode is now on Steroids. CrewCrewCrewCrew..."
  }
  ...
  return {
    event: ({ event }: { event: { type: string; properties?: unknown } }) => {
      ...
      setTimeout(async () => {
        ...
        if (localDevVersion) {
          if (showStartupToast) {
            showLocalDevToast(ctx, displayVersion, isCaptainEnabled).catch(() => {})
          }
