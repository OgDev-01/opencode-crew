...
describe("category-skill-reminder hook", () => {
  ...
  describe("target agent detection", () => {
    test("should inject reminder for captain agent after 3 tool calls", async () => {
      // given - captain agent session with multiple tool calls
    ...
    test("should inject reminder for relay agent", async () => {
      // given - relay agent session
      const hook = createHook()
      const sessionID = "relay-session"
      updateSessionAgent(sessionID, "Relay")
    ...
    test("should inject reminder for cadet agent", async () => {
      // given - cadet agent session
      const hook = createHook()
    ...
    test("should NOT inject reminder for non-target agents", async () => {
      // given - archivist agent session (not a target)
      const hook = createHook()
      const sessionID = "archivist-session"
      updateSessionAgent(sessionID, "archivist")
    ...
    test("should detect agent from input.agent when session state is empty", async () => {
      ...
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "1", agent: "Captain" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "2", agent: "Captain" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "3", agent: "Captain" }, output)
  ...
  describe("delegation tool tracking", () => {
    test("should NOT inject reminder if task is used", async () => {
      // given - captain agent that uses task
      const hook = createHook()
    ...
    test("should NOT inject reminder if task tool is used", async () => {
      // given - captain agent that uses task tool
      const hook = createHook()
  ...
  describe("tool call counting", () => {
    test("should NOT inject reminder before 3 tool calls", async () => {
      // given - captain agent with only 2 tool calls
      const hook = createHook()
    ...
    test("should only count delegatable work tools", async () => {
      // given - captain agent with mixed tool calls
      const hook = createHook()
  ...
  describe("event handling", () => {
    test("should reset state on session.deleted event", async () => {
      // given - captain agent with reminder already shown
      const hook = createHook()
  ...
  describe("case insensitivity", () => {
    test("should handle tool names case-insensitively", async () => {
      // given - captain agent with mixed case tool names
      const hook = createHook()
    ...
    test("should handle delegation tool names case-insensitively", async () => {
      // given - captain agent using TASK in uppercase
      const hook = createHook()
