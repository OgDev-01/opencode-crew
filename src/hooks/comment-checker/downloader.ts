...
import { cleanupArchive,
downloadArchive,
ensureCacheDir,
ensureExecutable,
extractTarGz,
extractZipArchive,
getCachedBinaryPath as getCachedBinaryPathShared, } from "@/shared/binary/binary-downloader"
...
/**
 * Get the cache directory for opencode-crew binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "opencode-crew", "bin")
  }
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[opencode-crew] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[opencode-crew] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[opencode-crew] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[opencode-crew] Comment checking disabled.`)
    return null
  }
}
