...
describe("captain-task-retry", () => {
  ...
  describe("detectDelegateTaskError", () => {
    ...
    it("should detect unknown agent error", () => {
      const output = '[ERROR] Unknown agent: "fake-agent". Available agents: lookout, archivist, sage'
  ...
  describe("buildRetryGuidance", () => {
    ...
    it("should provide fix for unknown agent with available list", () => {
      const errorInfo = { 
        ...
        originalOutput: '[ERROR] Unknown agent: "fake". Available agents: lookout, sage' 
      }
      
      const guidance = buildRetryGuidance(errorInfo)
      
      expect(guidance).toContain("lookout")
      expect(guidance).toContain("sage")
    })
  })
})
