...
Please lookout the codebase and discover patterns.
</system-reminder>`
      }],
    }
...
describe("keyword-detector agent-specific ultrawork messages", () => {
  ...
  test("should skip ultrawork injection when agent is strategist", async () => {
    ...
    // when - ultrawork keyword detected with strategist agent
  ...
  test("should skip ultrawork injection when agent name contains 'planner'", async () => {
    ...
    await hook["chat.message"]({ sessionID, agent: "Strategist (Planner)" }, output)
  ...
  test("should use normal ultrawork message when agent is Captain", async () => {
    // given - collector and Captain agent
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    ...
    // when - ultrawork keyword detected with Captain agent
    await hook["chat.message"]({ sessionID, agent: "captain" }, output)
  ...
  test("should skip ultrawork for strategist but inject for captain", async () => {
    // given - two sessions, one with strategist, one with captain
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    ...
    await hook["chat.message"]({ sessionID: strategistSessionID, agent: "strategist" }, strategistOutput)

    // Second session with captain
    const captainSessionID = "captain-second"
    const captainOutput = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrawork implement" }],
    }
    await hook["chat.message"]({ sessionID: captainSessionID, agent: "captain" }, captainOutput)

    // then - strategist should have no injection, captain should have normal ultrawork
    const strategistTextPart = strategistOutput.parts.find(p => p.type === "text")
    expect(strategistTextPart!.text).toBe("ultrawork plan")

    const captainTextPart = captainOutput.parts.find(p => p.type === "text")
    expect(captainTextPart!.text).toContain("YOU MUST LEVERAGE ALL AVAILABLE AGENTS")
    expect(captainTextPart!.text).toContain("---")
    expect(captainTextPart!.text).toContain("implement")
  })

  test("should use session state agent over stale input.agent (bug fix)", async () => {
    // given - same session, agent switched from strategist-to-captain in session state
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    const sessionID = "same-session-agent-switch"

    // Simulate: session state was updated to captain (by index.ts updateSessionAgent)
    ...
    // when - hook receives stale input.agent="strategist" but session state says "Captain"
    await hook["chat.message"]({ sessionID, agent: "strategist" }, output)

    // then - should use Captain from session state, NOT strategist from stale input
    const textPart = output.parts.find(p => p.type === "text")
    expect(textPart).toBeDefined()
    expect(textPart!.text).toContain("YOU MUST LEVERAGE ALL AVAILABLE AGENTS")
    expect(textPart!.text).not.toContain("YOU ARE A PLANNER, NOT AN IMPLEMENTER")
  ...
  test("should fall back to input.agent when session state is empty and skip ultrawork for strategist", async () => {
    ...
    // then - strategist fallback from input.agent, ultrawork skipped
    const textPart = output.parts.find(p => p.type === "text")
    expect(textPart).toBeDefined()
