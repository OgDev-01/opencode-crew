 ...
 *
 * Key characteristics:
 * - Natural tool-like usage of lookout/archivist agents (run_in_background=true)
...
2. **EXPLORE THOROUGHLY** - Fire lookout/archivist agents to gather ALL relevant context
3. **CONSULT SPECIALISTS** - For hard/complex tasks, DO NOT struggle alone. Delegate:
   - **Sage**: Conventional problems - architecture, debugging, complex logic
...
task(subagent_type="sage", load_skills=[], prompt="I need architectural review of my approach to [TASK]. Here's my plan: [DESCRIBE PLAN WITH SPECIFIC FILES AND CHANGES]. My concerns are: [LIST SPECIFIC UNCERTAINTIES]. Please evaluate: correctness of approach, potential issues I'm missing, and whether a better alternative exists.", run_in_background=false)
\`\`\`
...
3. **DO** consult specialists (sage for conventional, artistry for non-conventional)
4. **DO** ask the user for guidance
5. **DO** lookout alternative approaches
...
|-----------|--------|-----|
| Codebase exploration | task(subagent_type="lookout", load_skills=[], run_in_background=true) | Parallel, context-efficient |
| Documentation lookup | task(subagent_type="archivist", load_skills=[], run_in_background=true) | Specialized knowledge |
| Planning | task(subagent_type="plan", load_skills=[]) | Parallel task graph + structured TODO list |
| Hard problem (conventional) | task(subagent_type="sage", load_skills=[]) | Architecture, debugging, complex logic |
...
2. Spawn exploration/archivist agents via task(run_in_background=true) in PARALLEL (10+ if needed)
