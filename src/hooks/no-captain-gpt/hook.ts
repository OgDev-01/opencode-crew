...
const TOAST_TITLE = "NEVER Use Captain with GPT"
const TOAST_MESSAGE = [
  "Captain works best with Claude Opus, and works fine with Kimi/GLM models.",
  "Do NOT use Captain with GPT.",
...
function showToast(ctx: PluginInput, sessionID: string): void {
  ctx.client.tui.showToast({
    ...
    log("[no-captain-gpt] Failed to show toast", {
      sessionID,
      error,
    })
  })
...
export function createNoCaptainGptHook(ctx: PluginInput) {
  return {
    "chat.message": async (input: {
      ...
      if (agentKey === "captain" && modelID && isGptModel(modelID)) {
