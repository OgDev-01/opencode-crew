...
describe("preemptive-compaction", () => {
  ...
  // #given preemptive compaction hook
  // #when context usage reaches default threshold 0.70
  // #then compaction is triggered
  it("should trigger compaction when usage reaches config threshold of 0.70", async () => {
    // Default config has threshold 0.70
    const defaultConfig = {}
    const hook = createPreemptiveCompactionHook(ctx as never, defaultConfig as never)
    const sessionID = "ses_threshold_0_70"

    // 140K input + 10K cache = 150K → 75% of 200K (above 0.70 threshold)
    await hook.event({
      event: {
        type: "message.updated",
        properties: {
  ...
  // #given preemptive compaction config has threshold 0.80
  // #when context usage reaches 0.75 (below 0.80)
  // #then compaction is NOT triggered
  it("should not trigger compaction when config threshold is 0.80 and usage is 0.75", async () => {
    const configWith80Threshold = {
      preemptive_compaction: { threshold: 0.8 },
    }
    const hook = createPreemptiveCompactionHook(ctx as never, configWith80Threshold as never)
    const sessionID = "ses_threshold_0_80"

    // 150K input + 10K cache = 160K → 80% of 200K
    // With threshold 0.80, this is EXACTLY at threshold, so should trigger
    // Use 75% to stay below
    // 140K + 10K = 150K → 75% of 200K (below 0.80 threshold)
    await hook.event({
      event: {
        type: "message.updated",
        properties: {
    ...
    // At 75%, with threshold 0.80, should NOT trigger
    expect(ctx.client.session.summarize).not.toHaveBeenCalled()
  })
