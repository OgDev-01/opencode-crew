...
/ralph-loop → startLoop(sessionID, prompt, options)
  → loopState.startLoop() → persists state to .crew/ralph-loop.local.md
...
| `storage.ts` | Read/write `.crew/ralph-loop.local.md` state file |
...
```
.crew/ralph-loop.local.md  (gitignored)
