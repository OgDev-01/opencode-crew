...
export function createRelayEventHandler(input: {
  ctx: PluginInput
  options?: RelayHookOptions
  ...
  return async ({ event }): Promise<void> => {
    ...
    if (event.type === "session.idle") {
      ...
      const requiredAgent = getAgentConfigKey(boulderState.agent ?? "relay")
      const lastAgentMatchesRequired = lastAgentKey === requiredAgent
      const boulderAgentDefaultsToRelay = requiredAgent === "relay"
      const lastAgentIsCaptain = lastAgentKey === "captain"
      const allowCaptainForRelayBoulder = boulderAgentDefaultsToRelay && lastAgentIsCaptain
      const agentMatches = lastAgentMatchesRequired || allowCaptainForRelayBoulder
      if (!agentMatches) {
