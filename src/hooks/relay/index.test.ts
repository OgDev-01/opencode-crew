...
const TEST_STORAGE_ROOT = join(tmpdir(), `relay-message-storage-${randomUUID()}`)
...
mock.module("../../shared/opencode/opencode-message-dir", () => ({
...
mock.module("../../shared/opencode/opencode-storage-detection", () => ({
  isSqliteBackend: () => false,
}))

const { createRelayHook } = await import("./index")
...
describe("relay hook", () => {
  ...
  function createMockPluginInput(overrides?: { promptMock?: ReturnType<typeof mock> }) {
    ...
    } as unknown as Parameters<typeof createRelayHook>[0] & { _promptMock: ReturnType<typeof mock> }
  }
  ...
  beforeEach(() => {
    TEST_DIR = join(tmpdir(), `relay-test-${randomUUID()}`)
    CREW_DIR = join(TEST_DIR, ".crew")
    if (!existsSync(TEST_DIR)) {
      mkdirSync(TEST_DIR, { recursive: true })
    }
    if (!existsSync(CREW_DIR)) {
  ...
  describe("tool.execute.after handler", () => {
    test("should handle undefined output gracefully (issue #1035)", async () => {
      ...
      const hook = createRelayHook(createMockPluginInput())
    ...
    test("should ignore non-task tools", async () => {
     ...
     test("should not transform when caller is not Relay", async () => {
       // given - boulder state exists but caller agent in message storage is not Relay
     ...
     test("should append standalone verification when no boulder state but caller is Relay", async () => {
       // given - no boulder state, but caller is Relay
       const sessionID = "session-no-boulder-test"
       setupMessageStorage(sessionID, "relay")
     ...
     test("should transform output when caller is Relay with boulder state", async () => {
       // given - Relay caller with boulder state
     ...
     test("should still transform when plan is complete (shows progress)", async () => {
       // given - boulder state with complete plan, Relay caller
     ...
     test("should append session ID to boulder state if not present", async () => {
       // given - boulder state without session-append-test, Relay caller
     ...
     test("should not duplicate existing session ID", async () => {
       // given - boulder state already has session-dup-test, Relay caller
     ...
     test("should include boulder.json path and notepad path in transformed output", async () => {
       // given - boulder state, Relay caller
    ...
    describe("Write/Edit tool direct work reminder", () => {
      const ORCHESTRATOR_SESSION = "orchestrator-write-test"
       ...
       beforeEach(() => {
         setupMessageStorage(ORCHESTRATOR_SESSION, "relay")
       })
      ...
      test("should append delegation reminder when orchestrator writes outside .crew/", async () => {
        // given
        const hook = createRelayHook(createMockPluginInput())
        const output = {
          title: "Write",
      ...
      test("should append delegation reminder when orchestrator edits outside .crew/", async () => {
        // given
        const hook = createRelayHook(createMockPluginInput())
        const output = {
          title: "Edit",
      ...
      test("should NOT append reminder when orchestrator writes inside .crew/", async () => {
        // given
        const hook = createRelayHook(createMockPluginInput())
        ...
        const output = {
          ...
          metadata: { filePath: "/project/.crew/plans/work-plan.md" },
        }
      ...
      test("should NOT append reminder when non-orchestrator writes outside .crew/", async () => {
        // given
      ...
      describe("cross-platform path validation (Windows support)", () => {
        test("should NOT append reminder when orchestrator writes inside .crew\\ (Windows backslash)", async () => {
          // given
          const hook = createRelayHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: ".crew\\plans\\work-plan.md" },
          }
        ...
        test("should NOT append reminder when orchestrator writes inside .crew with mixed separators", async () => {
          // given
          const hook = createRelayHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
        ...
        test("should NOT append reminder for absolute Windows path inside .crew\\", async () => {
          // given
          const hook = createRelayHook(createMockPluginInput())
          const originalOutput = "File written successfully"
          const output = {
            title: "Write",
            output: originalOutput,
            metadata: { filePath: "C:\\Users\\test\\project\\.crew\\plans\\x.md" },
          }
        ...
        test("should append reminder for Windows path outside .crew\\", async () => {
          // given
          const hook = createRelayHook(createMockPluginInput())
          const output = {
            title: "Write",
  ...
  describe("session.idle handler (boulder continuation)", () => {
    ...
    async function flushMicrotasks(): Promise<void> {
     ...
     beforeEach(() => {
       ...
       setupMessageStorage(MAIN_SESSION_ID, "relay")
     })
    ...
    test("should inject continuation when boulder has incomplete tasks", async () => {
      ...
      const hook = createRelayHook(mockInput)
    ...
    test("should inject when last agent is captain and boulder targets relay explicitly", async () => {
       // given - boulder explicitly set to relay, but last agent is captain (initial state after /start-work)
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       // given - last agent is captain (typical state right after /start-work)
       cleanupMessageStorage(MAIN_SESSION_ID)
       setupMessageStorage(MAIN_SESSION_ID, "captain")
       ...
       // then - should call prompt because captain is always allowed for relay boulders
       expect(mockInput._promptMock).toHaveBeenCalled()
     })

     test("should not inject when last agent is non-captain and does not match boulder agent", async () => {
       // given - boulder explicitly set to relay, last agent is craftsman (unrelated agent)
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       cleanupMessageStorage(MAIN_SESSION_ID)
       setupMessageStorage(MAIN_SESSION_ID, "craftsman")
       ...
       // then - should NOT call prompt because craftsman does not match relay or captain
       expect(mockInput._promptMock).not.toHaveBeenCalled()
     })

     test("should inject when last agent matches boulder agent even if non-Relay", async () => {
       // given - boulder state expects captain and last agent is captain
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       // then - should call prompt for captain
       expect(mockInput._promptMock).toHaveBeenCalled()
       const callArgs = mockInput._promptMock.mock.calls[0][0]
       expect(callArgs.body.agent).toBe("captain")
     })
    ...
    test("should inject when session agent was updated to relay by start-work even if message storage agent differs", async () => {
      // given - boulder targets relay, but nearest stored message still says craftsman
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
      ...
      cleanupMessageStorage(MAIN_SESSION_ID)
      setupMessageStorage(MAIN_SESSION_ID, "craftsman")
      updateSessionAgent(MAIN_SESSION_ID, "relay")
      ...
      // then - should continue because start-work updated session agent to relay
      expect(mockInput._promptMock).toHaveBeenCalled()
    })
  })
})
