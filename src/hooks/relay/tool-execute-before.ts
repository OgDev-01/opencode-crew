...
import { isCallerOrchestrator } from "@/features/hook-message-injector"
import type { PluginInput } from "@opencode-ai/plugin"
import { HOOK_NAME } from "./hook-name"
import { ORCHESTRATOR_DELEGATION_REQUIRED, SINGLE_TASK_DIRECTIVE } from "./system-reminder-templates"
import { isCrewPath } from "./crew-path"
...
export function createToolExecuteBeforeHandler(input: {
  ...
  return async (toolInput, toolOutput): Promise<void> => {
    ...
    // Warn-only policy: Relay guides orchestrators toward delegation but doesn't block, allowing flexibility for urgent fixes
    if (isWriteOrEditToolName(toolInput.tool)) {
      ...
      if (filePath && !isCrewPath(filePath)) {
