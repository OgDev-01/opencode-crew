...
describe("runtime-fallback", () => {
  ...
  function createMockPluginConfigWithCategoryFallback(fallbackModels: string[]): OpenCodeCrewConfig {
    return {
      categories: {
        test: {
  ...
  describe("fallback models configuration", () => {
    function createMockPluginConfigWithAgentFallback(agentName: string, fallbackModels: string[]): OpenCodeCrewConfig {
      return {
        agents: {
          [agentName]: {
            fallback_models: fallbackModels,
    ...
    test("should use agent-level fallback_models", async () => {
      ...
      const hook = createRuntimeFallbackHook(input, {
        config: createMockConfig({ notify_on_fallback: false }),
        pluginConfig: createMockPluginConfigWithAgentFallback("sage", ["openai/gpt-5.2", "google/gemini-3.1-pro"]),
      })
      ...
      await hook.event({
        event: {
          type: "session.created",
          properties: { info: { id: sessionID, model: "anthropic/claude-opus-4-5", agent: "sage" } },
        },
      })
      ...
      await hook.event({
        event: {
          type: "session.error",
          properties: { sessionID, error: { statusCode: 503 }, agent: "sage" },
        },
      })
    ...
    test("should detect agent from sessionID pattern", async () => {
      const hook = createRuntimeFallbackHook(createMockPluginInput(), {
        config: createMockConfig({ notify_on_fallback: false }),
        pluginConfig: createMockPluginConfigWithAgentFallback("captain", ["openai/gpt-5.2"]),
      })
      const sessionID = "captain-session-123"
      ...
      //#then - should detect captain from sessionID and use its fallback
      const fallbackLog = logCalls.find((c) => c.msg.includes("Preparing fallback"))
      expect(fallbackLog).toBeDefined()
    ...
    test("should preserve resolved agent during auto-retry", async () => {
      ...
      const hook = createRuntimeFallbackHook(
        ...
        {
          config: createMockConfig({ notify_on_fallback: false }),
          pluginConfig: createMockPluginConfigWithAgentFallback("strategist", ["github-copilot/claude-opus-4.6"]),
        },
      )
      ...
      expect(callBody?.agent).toBe("strategist")
