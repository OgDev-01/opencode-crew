...
describe("start-work hook", () => {
  ...
  describe("chat.message handler", () => {
    ...
    test("should auto-select when only one incomplete plan among multiple plans", async () => {
      ...
      const plansDir = join(testDir, ".crew", "plans")
...
<user-request>feature-implementation</user-request>
  ...
  describe("session agent management", () => {
    test("should update session agent to Relay when start-work command is triggered", async () => {
      // given
      ...
      // when
      await hook["chat.message"](
        { sessionID: "ses-strategist-to-captain" },
        output
      )

      // then
      expect(updateSpy).toHaveBeenCalledWith("ses-strategist-to-captain", "relay")
