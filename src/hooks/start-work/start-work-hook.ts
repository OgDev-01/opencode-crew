...
3. Update \`.crew/boulder.json\` — add \`"worktree_path": "<absolute-path>"\`
...
export function createStartWorkHook(ctx: PluginInput) {
  return {
    "chat.message": async (input: StartWorkHookInput, output: StartWorkHookOutput): Promise<void> => {
      ...
      updateSessionAgent(input.sessionID, "relay")
      ...
      if (explicitPlanName) {
        ...
        const allPlans = findStrategistPlans(ctx.directory)
...
All ${progress.total} tasks are done. Create a new plan with: /plan "your task"`
            ...
            const newState = createBoulderState(matchedPlan, sessionId, "relay", worktreePath)
...
Looking for new plans...`
      ...
      if (
        ...
        const plans = findStrategistPlans(ctx.directory)
...
No Strategist plan files found at .crew/plans/
Use Strategist to create a work plan first: /plan "your task"`
...
All ${plans.length} plan(s) are complete. Create a new plan with: /plan "your task"`
          ...
          const newState = createBoulderState(planPath, sessionId, "relay", worktreePath)
          writeBoulderState(ctx.directory, newState)
