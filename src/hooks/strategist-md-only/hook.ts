...
export function createStrategistMdOnlyHook(ctx: PluginInput) {
  return {
    "tool.execute.before": async (
      ...
      // Inject read-only warning for task tools called by Strategist
      ...
      if (!filePath) {
       ...
       if (!isAllowedFile(filePath, ctx.directory)) {
         log(`[${HOOK_NAME}] Blocked: Strategist can only write to .crew/*.md`, {
           sessionID: input.sessionID,
           tool: toolName,
           filePath,
           agent: agentName,
         ...
         throw new Error(
           `[${HOOK_NAME}] ${getAgentDisplayName("strategist")} can only write/edit .md files inside .crew/ directory. ` +
           `Attempted to modify: ${filePath}. ` +
           `${getAgentDisplayName("strategist")} is a READ-ONLY planner. Use /start-work to execute the plan. ` +
           `APOLOGIZE TO THE USER, REMIND OF YOUR PLAN WRITING PROCESSES, TELL USER WHAT YOU WILL GOING TO DO AS THE PROCESS, WRITE THE PLAN`
         )
       }

      const normalizedPath = filePath.toLowerCase().replace(/\\/g, "/")
      if (normalizedPath.includes(".crew/plans/") || normalizedPath.includes(".crew\\plans\\")) {
      ...
      log(`[${HOOK_NAME}] Allowed: .crew/*.md write permitted`, {
        sessionID: input.sessionID,
        tool: toolName,
        filePath,
        agent: agentName,
