...
import { SYSTEM_DIRECTIVE_PREFIX } from "../../shared/hook-utils/system-directive"
...
describe("strategist-md-only", () => {
  ...
  describe("agent name matching", () => {
    ...
    test("should not enforce restriction for non-Strategist agent", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "captain")
      const hook = createStrategistMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
        sessionID: TEST_SESSION_ID,
   ...
   describe("with Strategist agent in message storage", () => {
    ...
    test("should allow Strategist to write .md files inside .crew/", async () => {
      // given
      const hook = createStrategistMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      const output = {
        args: { filePath: "/tmp/test/.crew/plans/work-plan.md" },
      }
    ...
    test("should inject workflow reminder when Strategist writes to .crew/plans/", async () => {
      // given
      const hook = createStrategistMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      expect(output.message).toContain("ASSESSOR CONSULTATION")
      expect(output.message).toContain("CRITIC REVIEW")
    })

    test("should NOT inject workflow reminder for .crew/drafts/", async () => {
      // given
      const hook = createStrategistMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      const output: { args: Record<string, unknown>; message?: string } = {
        args: { filePath: "/tmp/test/.crew/drafts/notes.md" },
      }
    ...
    test("should block Strategist from writing .md files outside .crew/", async () => {
      // given
      const hook = createStrategistMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      await expect(
        hook["tool.execute.before"](input, output)
      ).rejects.toThrow("can only write/edit .md files inside .crew/")
    })
    ...
    test("should inject read-only warning when Strategist calls call_agent", async () => {
      // given
      const hook = createStrategistMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "call_agent",
  ...
  describe("boulder state priority over message files (fixes #927)", () => {
    ...
    const BOULDER_FILE = join(BOULDER_DIR, ".crew", "boulder.json")

    beforeEach(() => {
      mkdirSync(join(BOULDER_DIR, ".crew"), { recursive: true })
    })
    ...
    //#given session was started with strategist (first message), but /start-work set boulder agent to relay
    //#when user types "continue" after interruption (memory cleared, falls back to message files)
    //#then should use boulder state agent (relay), not message file agent (strategist)
    test("should prioritize boulder agent over message file agent", async () => {
      // given - strategist in message files (from /plan)
      setupMessageStorage(TEST_SESSION_ID, "strategist")
      
      // given - relay in boulder state (from /start-work)
      ...
      // when / then - should NOT block because boulder says relay, not strategist
      await expect(
        hook["tool.execute.before"](input, output)
      ).resolves.toBeUndefined()
    })

    test("should use strategist from boulder state when set", async () => {
      // given - relay in message files (from some other agent)
      setupMessageStorage(TEST_SESSION_ID, "relay")
      
      // given - strategist in boulder state (edge case, but should honor it)
      writeFileSync(BOULDER_FILE, JSON.stringify({
        active_plan: "/test/plan.md",
        started_at: new Date().toISOString(),
        session_ids: [TEST_SESSION_ID],
      ...
      // when / then - should block because boulder says strategist
      await expect(
        hook["tool.execute.before"](input, output)
      ).rejects.toThrow("can only write/edit .md files")
    })
  ...
  describe("cross-platform path validation", () => {
    beforeEach(() => {
     ...
     test("should allow Windows-style backslash paths under .crew/", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "strategist")
       const hook = createStrategistMdOnlyHook(createMockPluginInput())
       const input = {
       ...
       const output = {
         args: { filePath: ".crew\\plans\\work-plan.md" },
       }
     ...
     test("should allow mixed separator paths under .crew/", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "strategist")
       const hook = createStrategistMdOnlyHook(createMockPluginInput())
       const input = {
     ...
     test("should block paths outside workspace root even if containing .crew", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "strategist")
       const hook = createStrategistMdOnlyHook(createMockPluginInput())
       const input = {
       ...
       const output = {
         args: { filePath: "/other/project/.crew/plans/x.md" },
       }
     ...
     test("should allow nested .crew directories (ctx.directory may be parent)", async () => {
       ...
       const output = {
         args: { filePath: "src/.crew/plans/x.md" },
       }

       // when / #then - should allow because .crew is in path
       await expect(
         hook["tool.execute.before"](input, output)
       ).resolves.toBeUndefined()
     })
     ...
     test("should allow case-insensitive .CREW directory", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "strategist")
       const hook = createStrategistMdOnlyHook(createMockPluginInput())
       const input = {
     ...
     test("should allow nested project path with .crew (Windows real-world case)", async () => {
       // given - simulates when ctx.directory is parent of actual project
       // User reported: xauusd-dxy-plan\.crew\drafts\supabase-email-templates.md
       setupMessageStorage(TEST_SESSION_ID, "strategist")
       const hook = createStrategistMdOnlyHook(createMockPluginInput())
       const input = {
         tool: "Write",
       ...
       const output = {
         args: { filePath: "xauusd-dxy-plan\\.crew\\drafts\\supabase-email-templates.md" },
       }
     ...
     test("should allow nested project path with mixed separators", async () => {
       ...
       const output = {
         args: { filePath: "my-project/.crew\\plans/task.md" },
       }
     ...
     test("should block nested project path without .crew", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "strategist")
       const hook = createStrategistMdOnlyHook(createMockPluginInput())
       const input = {
