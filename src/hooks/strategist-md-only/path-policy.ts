 ...
 * - Windows backslashes (e.g., .crew\\plans\\x.md)
 * - Mixed separators (e.g., .crew\\plans/x.md)
 * - Case-insensitive directory/extension matching
 * - Workspace confinement (blocks paths outside root or via traversal)
 * - Nested project paths (e.g., parent/.crew/... when ctx.directory is parent)
 */
export function isAllowedFile(filePath: string, workspaceRoot: string): boolean {
  ...
  // 4. Check if .crew/ or .crew\ exists anywhere in the path (case-insensitive)
  // This handles both direct paths (.crew/x.md) and nested paths (project/.crew/x.md)
  if (!/\.crew[/\\]/i.test(rel)) {
    return false
  }
