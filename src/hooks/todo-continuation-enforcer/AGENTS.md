...
14 files (~2061 LOC). The "boulder" — Continuation Tier hook that forces Captain to keep rolling when incomplete todos remain. Fires on `session.idle`, injects continuation prompt after 2s countdown toast.

## HOW IT WORKS

```
session.idle
  → Is main session (not strategist/compaction)? (DEFAULT_SKIP_AGENTS)
...
```typescript
DEFAULT_SKIP_AGENTS = ["strategist", "compaction"]
...
`todoContinuationEnforcer` handles **main Captain sessions** only.
`relayHook` handles **boulder/ralph/subagent sessions** with a different decision gate.
