...
describe("todo-continuation-enforcer", () => {
  ...
  test("should accept skipAgents option without error", async () => {
    // given - session with skipAgents configured for Strategist
    const sessionID = "main-strategist-option"
    setMainSession(sessionID)

    // when - create hook with skipAgents option (should not throw)
    const hook = createTodoContinuationEnforcer(createMockPluginInput(), {
      skipAgents: ["Strategist (Planner)", "custom-agent"],
    })
  ...
  test("should extract model from assistant message with flat modelID/providerID", async () => {
    ...
    const mockMessagesWithAssistant = [
      { info: { id: "msg-1", role: "user", agent: "captain", model: { providerID: "openai", modelID: "gpt-5.2" } } },
      { info: { id: "msg-2", role: "assistant", agent: "captain", modelID: "gpt-5.2", providerID: "openai" } },
    ]
  ...
  test("should skip compaction agent messages when resolving agent info", async () => {
    // given - session where last message is from compaction agent but previous was Captain
    const sessionID = "main-compaction-filter"
    setMainSession(sessionID)

    const mockMessagesWithCompaction = [
      { info: { id: "msg-1", role: "user", agent: "captain", model: { providerID: "anthropic", modelID: "claude-sonnet-4-6" } } },
      { info: { id: "msg-2", role: "assistant", agent: "captain", modelID: "claude-sonnet-4-6", providerID: "anthropic" } },
    ...
    const mockInput = {
     ...
     // then - continuation uses Captain (skipped compaction agent)
     expect(promptCalls.length).toBe(1)
    expect(promptCalls[0].agent).toBe("captain")
  })
  ...
  test("should skip injection when strategist agent is after compaction", async () => {
    // given - strategist session that was compacted
    const sessionID = "main-strategist-compacted"
    setMainSession(sessionID)

    const mockMessagesStrategistCompacted = [
      { info: { id: "msg-1", role: "user", agent: "strategist" } },
      { info: { id: "msg-2", role: "assistant", agent: "strategist" } },
    ...
    const mockInput = {
      client: {
        session: {
          todo: async () => ({
           ...
           messages: async () => ({ data: mockMessagesStrategistCompacted }),
           prompt: async (opts: any) => {
             promptCalls.push({
               sessionID: opts.path.id,
               agent: opts.body.agent,
     ...
     // then - no continuation (strategist found after filtering compaction, strategist is in skipAgents)
    expect(promptCalls).toHaveLength(0)
  })
