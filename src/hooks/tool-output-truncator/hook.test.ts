...
import { createToolOutputTruncatorHook } from "./hook"
import * as dynamicTruncator from "../../shared/type-utils/dynamic-truncator"
...
describe("createToolOutputTruncatorHook", () => {
  ...
  describe("tool.execute.after", () => {
    ...
    describe("#given NEVER_TRUNCATE_TOOLS guard", () => {
      describe("#when truncate_all_tool_outputs is true and tool is in NEVER_TRUNCATE_TOOLS", () => {
        it("#then output is NOT truncated (read tool protected)", async () => {
          const truncateMock = mock(async () => ({
            result: "truncated",
            truncated: true,
          }))
        ...
        it("#then output is NOT truncated (lsp_rename tool protected)", async () => {
          const truncateMock = mock(async () => ({
            result: "truncated",
            truncated: true,
          }))
          ...
          const input = createInput("lsp_rename")
          const output = createOutput("rename result")
          const originalOutput = output.output
      ...
      describe("#when truncate_all_tool_outputs is true and tool is NOT in NEVER_TRUNCATE_TOOLS", () => {
        it("#then output IS truncated (bash tool not protected)", async () => {
          const truncateMock = mock(async (_sessionID: string, _output: string, options?: { targetMaxTokens?: number }) => ({
          ...
          const output = createOutput("large bash output")

          await hook["tool.execute.after"](input, output)

          expect(truncateMock).toHaveBeenCalled()
          expect(output.output).toBe("truncated output")
        })
      })

      describe("#when truncate_all_tool_outputs is false and tool is NOT in TRUNCATABLE_TOOLS", () => {
        it("#then output is NOT truncated (original behavior preserved)", async () => {
          const truncateMock = mock(async () => ({
            result: "truncated",
            truncated: true,
          }))
          ...
          hook = createToolOutputTruncatorHook({} as never, {
            experimental: { truncate_all_tool_outputs: false },
          })

          const input = createInput("write")
          const output = createOutput("output content")
          const originalOutput = output.output
