...
import { CREW_INTERNAL_INITIATOR_MARKER } from "../../shared/hook-utils/internal-initiator-marker"
...
describe("unstable-agent-babysitter hook", () => {
  ...
  test("fires reminder for hung gemini task", async () => {
    ...
    const ctx = createMockPluginInput({
      messagesBySession: {
        "main-1": [
          { info: { agent: "captain", model: { providerID: "openai", modelID: "gpt-4" } } },
        ],
        "bg-1": [
    ...
    expect(text).toContain(CREW_INTERNAL_INITIATOR_MARKER)
  })
