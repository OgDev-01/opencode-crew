...
| `strategist-agent-config-builder.ts` | ~100 | Strategist config with model resolution |
| `plan-model-inheritance.ts` | 28 | Plan demotion logic |
| `agent-priority-order.ts` | ~30 | captain, craftsman, strategist, relay first |
...
|-------|---------|--------|
| Archivist | grep_app_* | — |
| Relay, Captain, Strategist | task, task_*, teammate | — |
| Craftsman | task | — |
...
```
User (~/.config/opencode/opencode-crew.jsonc)
  ↓ deepMerge
Project (.opencode/opencode-crew.jsonc)
  ↓ Zod defaults
Final Config
```
