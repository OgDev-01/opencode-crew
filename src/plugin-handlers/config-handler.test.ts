...
import * as cadet from "../agents/cadet"
...
import * as configDir from "../shared/opencode/opencode-config-dir"
import * as permissionCompat from "../shared/file-ops/permission-compat"
import * as modelResolver from "../shared/model/model-resolver"

beforeEach(() => {
  spyOn(agents, "createBuiltinAgents" as any).mockResolvedValue({
    captain: { name: "captain", prompt: "test", mode: "primary" },
    sage: { name: "sage", prompt: "test", mode: "subagent" },
  })
...
afterEach(() => {
  ...
  ;(cadet.createCadetAgentWithOverrides as any)?.mockRestore?.()
...
describe("Cadet model inheritance", () => {
  test("does not inherit UI-selected model as system default", async () => {
    ...
    expect(agentConfig[getAgentDisplayName("cadet")]?.model).toBe(
      cadet.CADET_DEFAULTS.model
    )
  })

  test("uses explicitly configured cadet model", async () => {
    // #given
    const pluginConfig: OpenCodeCrewConfig = {
      agents: {
        "cadet": {
...
describe("Plan agent demote behavior", () => {
  test("orders core agents as captain -> craftsman -> strategist -> relay", async () => {
    // #given
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      captain: { name: "captain", prompt: "test", mode: "primary" },
      craftsman: { name: "craftsman", prompt: "test", mode: "primary" },
      sage: { name: "sage", prompt: "test", mode: "subagent" },
      relay: { name: "relay", prompt: "test", mode: "primary" },
    })
    const pluginConfig: OpenCodeCrewConfig = {
      captain_agent: {
    ...
    const coreAgents = [
      getAgentDisplayName("captain"),
      getAgentDisplayName("craftsman"),
      getAgentDisplayName("strategist"),
      getAgentDisplayName("relay"),
    ]
    const ordered = keys.filter((key) => coreAgents.includes(key))
    expect(ordered).toEqual(coreAgents)
  })

  test("plan agent should be demoted to subagent without inheriting strategist prompt", async () => {
    // #given
    const pluginConfig: OpenCodeCrewConfig = {
      captain_agent: {
        planner_enabled: true,
    ...
    // #then - plan is demoted to subagent but does NOT inherit strategist prompt
    ...
    expect(agents[getAgentDisplayName("strategist")]?.prompt).toBeDefined()
  })
  ...
  test("plan agent remains unchanged when planner is disabled", async () => {
    ...
    // #then - plan is not touched, strategist is not created
    const agents = config.agent as Record<string, { mode?: string; name?: string; prompt?: string }>
    expect(agents[getAgentDisplayName("strategist")]).toBeUndefined()
    expect(agents.plan).toBeDefined()
    expect(agents.plan.mode).toBe("primary")
    expect(agents.plan.prompt).toBe("original plan prompt")
  })

  test("strategist should have mode 'all' to be callable via task", async () => {
    // given
    const pluginConfig: OpenCodeCrewConfig = {
      captain_agent: {
        planner_enabled: true,
    ...
    const strategistKey = getAgentDisplayName("strategist")
    expect(agents[strategistKey]).toBeDefined()
    expect(agents[strategistKey].mode).toBe("all")
  })
})

describe("Agent permission defaults", () => {
  test("craftsman should allow task", async () => {
    // #given
    const createBuiltinAgentsMock = agents.createBuiltinAgents as unknown as {
      mockResolvedValue: (value: Record<string, unknown>) => void
    }
    ...
    const craftsmanKey = getAgentDisplayName("craftsman")
    expect(agentConfig[craftsmanKey]).toBeDefined()
    expect(agentConfig[craftsmanKey].permission?.task).toBe("allow")
  })
})

describe("default_agent behavior with Captain orchestration", () => {
  test("canonicalizes configured default_agent with surrounding whitespace", async () => {
    // given
    const pluginConfig: OpenCodeCrewConfig = {}
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
      default_agent: "  craftsman  ",
      agent: {},
    }
    const handler = createConfigHandler({
      ctx: { directory: "/tmp" },
    ...
    // then
    expect(config.default_agent).toBe(getAgentDisplayName("craftsman"))
  })
  ...
  test("preserves existing display-name default_agent", async () => {
    // #given
    const pluginConfig: OpenCodeCrewConfig = {}
    const displayName = getAgentDisplayName("craftsman")
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
  ...
  test("sets default_agent to captain when missing", async () => {
    // #given
    const pluginConfig: OpenCodeCrewConfig = {}
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
    ...
    // #then
    expect(config.default_agent).toBe(getAgentDisplayName("captain"))
  })

  test("sets default_agent to captain when configured default_agent is empty after trim", async () => {
    // given
    const pluginConfig: OpenCodeCrewConfig = {}
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
  ...
  test("does not normalize configured default_agent when Captain is disabled", async () => {
    // given
    const pluginConfig: OpenCodeCrewConfig = {
      captain_agent: {
        disabled: true,
    ...
    // then
    expect(config.default_agent).toBe("  CrAfTsMaN  ")
  })
})

describe("Strategist category config resolution", () => {
...
describe("Strategist direct override priority over category", () => {
  test("direct reasoningEffort takes priority over category reasoningEffort", async () => {
    ...
    const pKey = getAgentDisplayName("strategist")
  ...
  test("strategist prompt_append is appended to base prompt", async () => {
    // #given - strategist override with prompt_append
    ...
    const pKey = getAgentDisplayName("strategist")
    expect(agents[pKey]).toBeDefined()
    expect(agents[pKey].prompt).toContain("Strategist")
    expect(agents[pKey].prompt).toContain(customInstructions)
    expect(agents[pKey].prompt!.endsWith(customInstructions)).toBe(true)
  })
})

describe("Plan agent model inheritance from strategist", () => {
  test("plan agent inherits all model-related settings from resolved strategist config", async () => {
    //#given - strategist resolves to claude-opus-4-6 with model settings
    ...
    //#then - plan inherits model and variant from strategist, but NOT prompt
  ...
  test("plan agent inherits temperature, reasoningEffort, and other model settings from strategist", async () => {
    //#given - strategist configured with category that has temperature and reasoningEffort
    spyOn(shared, "resolveModelPipeline" as any).mockReturnValue({
      model: "openai/gpt-5.2",
      provenance: "override",
      variant: "high",
    ...
    //#then - plan inherits ALL model-related settings from resolved strategist
  ...
  test("plan agent user override takes priority over strategist inherited settings", async () => {
    //#given - strategist resolves to opus, but user has plan override for gpt-5.2
    spyOn(shared, "resolveModelPipeline" as any).mockReturnValue({
      model: "anthropic/claude-opus-4-6",
      provenance: "provider-fallback",
      variant: "max",
    ...
    //#then - plan uses its own override, not strategist settings
    const agents = config.agent as Record<string, Record<string, unknown>>
    expect(agents.plan.model).toBe("openai/gpt-5.2")
    expect(agents.plan.variant).toBe("high")
  ...
  test("plan agent does NOT inherit prompt, description, or color from strategist", async () => {
    //#given
    spyOn(shared, "resolveModelPipeline" as any).mockReturnValue({
      model: "anthropic/claude-opus-4-6",
      provenance: "provider-fallback",
...
describe("config-handler plugin loading error boundary (#1559)", () => {
  ...
  test("returns empty defaults when loadAllPluginComponents times out", async () => {
    ...
    const pluginConfig: OpenCodeCrewConfig = {
      experimental: { truncate_all_tool_outputs: false, plugin_load_timeout_ms: 100 },
    }
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
      agent: {},
...
describe("per-agent todowrite/todoread deny when task_system enabled", () => {
  ...
  test("denies todowrite and todoread for primary agents when task_system is enabled", async () => {
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      ...
      craftsman: { name: "craftsman", prompt: "test", mode: "primary" },
      relay: { name: "relay", prompt: "test", mode: "primary" },
      strategist: { name: "strategist", prompt: "test", mode: "primary" },
      "cadet": { name: "cadet", prompt: "test", mode: "subagent" },
      sage: { name: "sage", prompt: "test", mode: "subagent" },
    })

    const pluginConfig: OpenCodeCrewConfig = {
      experimental: { truncate_all_tool_outputs: false, task_system: true },
    }
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
      agent: {},
  ...
  test("does not deny todowrite/todoread when task_system is disabled", async () => {
    ...
    const pluginConfig: OpenCodeCrewConfig = {
      experimental: { truncate_all_tool_outputs: false, task_system: false },
    }
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
      agent: {},
    ...
    //#then
    const agentResult = config.agent as Record<string, { permission?: Record<string, unknown> }>
    expect(agentResult[getAgentDisplayName("captain")]?.permission?.todowrite).toBeUndefined()
    expect(agentResult[getAgentDisplayName("captain")]?.permission?.todoread).toBeUndefined()
    expect(agentResult[getAgentDisplayName("craftsman")]?.permission?.todowrite).toBeUndefined()
    expect(agentResult[getAgentDisplayName("craftsman")]?.permission?.todoread).toBeUndefined()
  })
...
describe("disable_crew_env pass-through", () => {
  test("passes disable_crew_env=true to createBuiltinAgents", async () => {
    //#given
    const createBuiltinAgentsMock = agents.createBuiltinAgents as unknown as {
      mockResolvedValue: (value: Record<string, unknown>) => void
      mock: { calls: unknown[][] }
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      captain: { name: "captain", prompt: "without-env", mode: "primary" },
    })

    const pluginConfig: OpenCodeCrewConfig = {
      experimental: { truncate_all_tool_outputs: false, disable_crew_env: true },
    }
    const config: Record<string, unknown> = {
      model: "anthropic/claude-opus-4-6",
      agent: {},
  ...
  test("passes disable_crew_env=false to createBuiltinAgents when omitted", async () => {
    //#given
    const createBuiltinAgentsMock = agents.createBuiltinAgents as unknown as {
      mockResolvedValue: (value: Record<string, unknown>) => void
      mock: { calls: unknown[][] }
    ...
    createBuiltinAgentsMock.mockResolvedValue({
      captain: { name: "captain", prompt: "with-env", mode: "primary" },
    })
