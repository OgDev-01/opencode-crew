...
export function createTransformHooks(args: {
  ...
  const memoryInjection = hookSlot("memory-injection", () => { try { const { getMemoryManager } = require("../../features/memory/manager"); const manager = getMemoryManager(); const maxInjectionTokens = pluginConfig.memory?.max_injection_tokens; return createMemoryInjectionHook({ search: manager.search, collector: contextCollector, getMainSessionID, getUsage: getSessionContextUsage, recordLearningConsulted: async (learning) => { await manager.storage.incrementTimesConsulted(learning.id) }, config: { enabled: pluginConfig.memory?.dynamic_prompts_enabled ?? true, maxTokens: maxInjectionTokens, goldenRuleMaxTokens: maxInjectionTokens ? Math.min(maxInjectionTokens, 200) : undefined, maxGoldenRules: pluginConfig.memory?.max_golden_rules_injected, maxLearnings: pluginConfig.memory?.max_learnings_injected, minRelevance: pluginConfig.memory?.similarity_threshold } }) } catch (error) { log(`Failed to initialize memory-injection hook: ${error instanceof Error ? error.message : String(error)}`); return null } }, isHookEnabled, safeHookEnabled)
