...
describe("createToolExecuteBeforeHandler", () => {
  ...
  describe("task tool subagent_type normalization", () => {
    ...
    test("sets subagent_type to cadet when category is provided without subagent_type", async () => {
      //#given
    ...
    test("sets subagent_type to cadet when category provided with different subagent_type", async () => {
      //#given
      const ctx = createCtxWithSessionMessages()
      const handler = createToolExecuteBeforeHandler({ ctx, hooks: emptyHooks })
      const input = { tool: "task", sessionID: "ses_123", callID: "call_1" }
      const output = { args: { category: "quick", subagent_type: "sage", description: "Test" } as Record<string, unknown> }
    ...
    test("resolves subagent_type from session first message when session_id provided without subagent_type", async () => {
      ...
      const ctx = createCtxWithSessionMessages([
        { info: { role: "user" } },
        { info: { role: "assistant", agent: "lookout" } },
        { info: { role: "assistant", agent: "sage" } },
      ])
      const handler = createToolExecuteBeforeHandler({ ctx, hooks: emptyHooks })
      const input = { tool: "task", sessionID: "ses_123", callID: "call_1" }
      ...
      //#then
      expect(output.args.subagent_type).toBe("lookout")
    })
    ...
    test("preserves subagent_type when session_id is provided with explicit subagent_type", async () => {
      ...
      const handler = createToolExecuteBeforeHandler({ ctx, hooks: emptyHooks })
      const input = { tool: "task", sessionID: "ses_123", callID: "call_1" }
      const output = { args: { session_id: "ses_abc123", subagent_type: "lookout", description: "Continue lookout" } as Record<string, unknown> }
    ...
    test("does not set subagent_type when neither category nor session_id is provided and subagent_type is present", async () => {
      ...
      const handler = createToolExecuteBeforeHandler({ ctx, hooks: emptyHooks })
      const input = { tool: "task", sessionID: "ses_123", callID: "call_1" }
      const output = { args: { subagent_type: "sage", description: "Sage task" } as Record<string, unknown> }

      //#when
      await handler(input, output)

      //#then
      expect(output.args.subagent_type).toBe("sage")
    })
  })
})
