...
describe("resolveUltraworkOverride", () => {
  ...
  test("should resolve override when ultrawork keyword detected", () => {
    //#given
    const config = createConfig("captain", { model: "anthropic/claude-opus-4-6", variant: "max" })
    const output = createOutput("ultrawork do something")

    //#when
    const result = resolveUltraworkOverride(config, "captain", output)
  ...
  test("should return null when no keyword detected", () => {
    //#given
    const config = createConfig("captain", { model: "anthropic/claude-opus-4-6" })
  ...
  test("should use message.agent when input agent is undefined", () => {
    //#given
    const config = createConfig("captain", { model: "anthropic/claude-opus-4-6" })
    const output = createOutput("ultrawork do something", "captain")
  ...
  test("should return null when agent has no ultrawork config", () => {
    //#given
    const config = {
      agents: { captain: { model: "anthropic/claude-sonnet-4-6" } },
    } as unknown as Parameters<typeof resolveUltraworkOverride>[0]
    const output = createOutput("ultrawork do something")
  ...
  test("should resolve variant-only override when ultrawork.model is not set", () => {
    //#given
    const config = createConfig("captain", { variant: "max" })
    const output = createOutput("ultrawork do something")
  ...
  test("should handle model string with multiple slashes", () => {
    //#given
    const config = createConfig("captain", { model: "openai/gpt-5.3/codex" })
    const output = createOutput("ultrawork do something")
  ...
  test("should return null when model string has no slash", () => {
    //#given
    const config = createConfig("captain", { model: "just-a-model" })
    const output = createOutput("ultrawork do something")
  ...
  test("should resolve display name to config key", () => {
    ...
    //#when
    const result = resolveUltraworkOverride(config, "Captain (Ultraworker)", output)
  ...
  test("should use session agent when input and message agents are undefined", () => {
    ...
    const config = createConfig("captain", { model: "anthropic/claude-opus-4-6", variant: "max" })
    const output = createOutput("ultrawork do something")
    const getSessionAgentSpy = spyOn(sessionStateModule, "getSessionAgent").mockReturnValue("captain")
...
describe("applyUltraworkModelOverrideOnMessage", () => {
  ...
  test("should schedule deferred DB override when message ID present", () => {
    ...
    //#when
    applyUltraworkModelOverrideOnMessage(config, "captain", output, tui)
  ...
  test("should override keyword-detector variant with configured ultrawork variant on deferred path", () => {
    //#given
    const config = createConfig("captain", {
  ...
  test("should apply variant-only override when no message ID", () => {
    //#given
    const config = createConfig("captain", { variant: "high" })
    const output = createOutput("ultrawork do something")
    const tui = createMockTui()
  ...
  test("should log the model transition with deferred DB tag", () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ agent: "captain" }),
    )
  })
  ...
  test("should resolve display name to config key with deferred path", () => {
    ...
    //#when
    applyUltraworkModelOverrideOnMessage(config, "Captain (Ultraworker)", output, tui)
