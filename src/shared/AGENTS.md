...
Cross-cutting utilities used throughout the plugin. Barrel-exported from `index.ts`. Logger writes to `/tmp/opencode-crew.log`.
...
- `agent-names.ts`: Old agent names → new (e.g., `junior` → `cadet`)
...
|---------|-------------|---------|
| `logger.ts` | 62 | `/tmp/opencode-crew.log` |
