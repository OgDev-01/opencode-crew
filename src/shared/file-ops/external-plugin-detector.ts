/**
 * Detects external plugins that may conflict with opencode-crew features.
...
import { parseJsoncSafe } from "../config/jsonc-parser"
...
/**
 * Known notification plugins that conflict with opencode-crew's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[opencode-crew] External notification plugin detected: ${pluginName}

Both opencode-crew and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   opencode-crew's session-notification has been auto-disabled.

   To use opencode-crew's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in opencode-crew.json`
}
