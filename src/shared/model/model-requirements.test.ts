...
describe("AGENT_MODEL_REQUIREMENTS", () => {
  test("sage has valid fallbackChain with gpt-5.2 as primary", () => {
    // given - sage agent requirement
    const sage = AGENT_MODEL_REQUIREMENTS["sage"]

    // when - accessing sage requirement
    // then - fallbackChain exists with gpt-5.2 as first entry
    expect(sage).toBeDefined()
    expect(sage.fallbackChain).toBeArray()
    expect(sage.fallbackChain.length).toBeGreaterThan(0)

    const primary = sage.fallbackChain[0]
    expect(primary.providers).toContain("openai")
    expect(primary.model).toBe("gpt-5.2")
    expect(primary.variant).toBe("high")
  })

  test("captain has claude-opus-4-6 as primary and requiresAnyModel", () => {
    // #given - captain agent requirement
    const captain = AGENT_MODEL_REQUIREMENTS["captain"]

    // #when - accessing Captain requirement
    // #then - fallbackChain has claude-opus-4-6 first, big-pickle last
    expect(captain).toBeDefined()
    expect(captain.fallbackChain).toBeArray()
    expect(captain.fallbackChain).toHaveLength(4)
    expect(captain.requiresAnyModel).toBe(true)

    const primary = captain.fallbackChain[0]
    expect(primary.providers).toEqual(["anthropic", "github-copilot", "opencode"])
    expect(primary.model).toBe("claude-opus-4-6")
    expect(primary.variant).toBe("max")

    const last = captain.fallbackChain[3]
    expect(last.providers[0]).toBe("opencode")
    expect(last.model).toBe("big-pickle")
  })

  test("archivist has valid fallbackChain with gemini-3-flash as primary", () => {
    // given - archivist agent requirement
    const archivist = AGENT_MODEL_REQUIREMENTS["archivist"]

    // when - accessing archivist requirement
    // then - fallbackChain exists with gemini-3-flash as first entry
    expect(archivist).toBeDefined()
    expect(archivist.fallbackChain).toBeArray()
    expect(archivist.fallbackChain.length).toBeGreaterThan(0)

    const primary = archivist.fallbackChain[0]
    expect(primary.providers[0]).toBe("google")
    expect(primary.model).toBe("gemini-3-flash")
  })

  test("lookout has valid fallbackChain with grok-code-fast-1 as primary", () => {
    // given - lookout agent requirement
    const lookout = AGENT_MODEL_REQUIREMENTS["lookout"]

    // when - accessing lookout requirement
    // then - fallbackChain: grok → minimax-free → haiku → nano
    expect(lookout).toBeDefined()
    expect(lookout.fallbackChain).toBeArray()
    expect(lookout.fallbackChain).toHaveLength(4)

    const primary = lookout.fallbackChain[0]
    expect(primary.providers).toContain("github-copilot")
    expect(primary.model).toBe("grok-code-fast-1")

    const secondary = lookout.fallbackChain[1]
    expect(secondary.providers).toContain("opencode")
    expect(secondary.model).toBe("minimax-m2.5-free")

    const tertiary = lookout.fallbackChain[2]
    expect(tertiary.providers).toContain("anthropic")
    expect(tertiary.model).toBe("claude-haiku-4-5")

    const quaternary = lookout.fallbackChain[3]
    expect(quaternary.providers).toContain("opencode")
    expect(quaternary.model).toBe("gpt-5-nano")
  })

  test("spotter has valid fallbackChain with kimi-k2.5-free as primary", () => {
  ...
  test("strategist has claude-opus-4-6 as primary", () => {
    ...
    expect(strategist.fallbackChain.length).toBeGreaterThan(1)
  ...
  test("assessor has claude-opus-4-6 as primary", () => {
    // #given - assessor agent requirement
    const assessor = AGENT_MODEL_REQUIREMENTS["assessor"]

    // #when - accessing Assessor requirement
    // #then - claude-opus-4-6 is first
    expect(assessor).toBeDefined()
    expect(assessor.fallbackChain).toBeArray()
    expect(assessor.fallbackChain.length).toBeGreaterThan(1)

    const primary = assessor.fallbackChain[0]
    expect(primary.model).toBe("claude-opus-4-6")
    expect(primary.providers).toEqual(["anthropic", "github-copilot", "opencode"])
    expect(primary.variant).toBe("max")
  })

  test("critic has valid fallbackChain with gpt-5.2 as primary", () => {
  ...
  test("relay has valid fallbackChain with kimi-k2.5-free as primary", () => {
    // given - relay agent requirement
    const relay = AGENT_MODEL_REQUIREMENTS["relay"]

    // when - accessing Relay requirement
    // then - fallbackChain exists with kimi-k2.5-free as first entry
    expect(relay).toBeDefined()
    expect(relay.fallbackChain).toBeArray()
    expect(relay.fallbackChain.length).toBeGreaterThan(0)

    const primary = relay.fallbackChain[0]
    expect(primary.model).toBe("kimi-k2.5-free")
    expect(primary.providers[0]).toBe("opencode")
  })

  test("craftsman supports openai, github-copilot, venice, and opencode providers", () => {
    // #given - craftsman agent requirement
    const craftsman = AGENT_MODEL_REQUIREMENTS["craftsman"]

    // #when - accessing craftsman requirement
    // #then - requiresProvider includes openai, github-copilot, venice, and opencode
    expect(craftsman).toBeDefined()
    expect(craftsman.requiresProvider).toEqual(["openai", "github-copilot", "venice", "opencode"])
    expect(craftsman.requiresModel).toBeUndefined()
  })
