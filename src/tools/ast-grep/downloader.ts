...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[opencode-crew] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[opencode-crew] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[opencode-crew] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[opencode-crew] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
