...
const projectDir = "/Users/yeongyu/local-workspaces/opencode-crew"
...
describe("background_output full_session", () => {
  test("resolves task_id into title metadata", async () => {
    ...
    expect(restored?.title).toBe("lookout - Find how task output is rendered")
  })

  test("shows category instead of agent for cadet", async () => {
    // #given
    clearPendingStore()
