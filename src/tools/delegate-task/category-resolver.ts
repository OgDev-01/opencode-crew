...
import type { FallbackEntry } from "@/shared/model/model-requirements"
import { mergeCategories } from "@/shared/config/merge-categories"
import { CADET_AGENT } from "./cadet-agent"
import { resolveCategoryConfig } from "./categories"
import { parseModelString } from "./model-string-parser"
import { CATEGORY_MODEL_REQUIREMENTS } from "@/shared/model/model-requirements"
...
export async function resolveCategoryExecution(
  ...
  const { client, userCategories, cadetModel } = executorCtx
...
2. Or configure an alternative model in your opencode-crew.json for this category
...
Available categories: ${allCategoryNames}`,
  ...
  const overrideModel = cadetModel
  const explicitCategoryModel = userCategories?.[args.category!]?.model

  if (!requirement) {
    // Precedence: explicit category model > cadet default > category resolved model
    // This keeps `cadet.model` useful as a global default while allowing
...
2. OpenCodeCrew: Set category model in opencode-crew.json
