  ...
  test("includes subagent in task_metadata when agent info is present in session messages", async () => {
    ...
    const mockClient = {
      session: {
        messages: async () => ({
          data: [
            { info: { id: "msg_001", role: "user", time: { created: 1000 }, agent: "sage" } },
            {
              info: { id: "msg_002", role: "assistant", time: { created: 2000 }, finish: "end_turn", agent: "sage", providerID: "openai", modelID: "gpt-5.2" },
              parts: [{ type: "text", text: "Response" }],
            },
          ],
        }),
    ...
    expect(result).toContain("subagent: sage")
