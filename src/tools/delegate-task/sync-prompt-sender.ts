...
} from "@/shared/model/model-suggestion-retry"
import { formatDetailedError } from "./error-formatting"
import { getAgentToolRestrictions } from "@/shared/agent/agent-tool-restrictions"
import { setSessionTools } from "@/shared/session/session-tools-store"
import { createInternalAgentTextPart } from "@/shared/hook-utils/internal-initiator-marker"
...
function isSageAgent(agentToUse: string): boolean {
  return agentToUse.toLowerCase() === "sage"
}
...
export async function sendSyncPrompt(
  ...
  try {
    ...
    if (isSageAgent(input.agentToUse) && isUnexpectedEofError(promptError)) {
      try {
        await deps.promptSyncWithModelSuggestionRetry(client, promptArgs)
        return null
      } catch (sageRetryError) {
        promptError = sageRetryError
      }
    }
