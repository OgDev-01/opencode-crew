...
import { __resetModelCache } from "../../shared/model/model-availability"
...
describe("captain-task", () => {
  ...
  describe("isPlanAgent", () => {
    ...
    test("returns false for case-insensitive match 'Strategist' (decoupled from plan)", () => {
      //#given / #when
      const result = isPlanAgent("Strategist")
    ...
    test("returns false for 'sage'", () => {
      // given / #when
      const result = isPlanAgent("sage")

      // then
      expect(result).toBe(false)
    })

    test("returns false for 'lookout'", () => {
      // given / #when
      const result = isPlanAgent("lookout")
  ...
  describe("isPlanFamily", () => {
    ...
    test("returns true for 'strategist'", () => {
      //#given / #when
      const result = isPlanFamily("strategist")
      //#then
      expect(result).toBe(true)
    })
    ...
    test("returns false for 'sage'", () => {
      //#given / #when
      const result = isPlanFamily("sage")
      //#then
      expect(result).toBe(false)
    })
    ...
    test("PLAN_FAMILY_NAMES contains plan and strategist", () => {
      //#given / #when / #then
      expect(PLAN_FAMILY_NAMES).toEqual(["plan", "strategist"])
    })
  })
  ...
  describe("category delegation config validation", () => {
    test("fills subagent_type as cadet when category is provided without subagent_type", async () => {
      // given
      const { createDelegateTask } = require("./tools")
    ...
    test("category overrides subagent_type and still maps to cadet", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
    ...
    test("proceeds without error when systemDefaultModel is undefined", async () => {
      ...
      const { createDelegateTask } = require("./tools")
       ...
       expect(result).not.toContain("opencode-crew requires a default model")
    }, { timeout: 10000 })
  ...
  describe("background metadata sessionId", () => {
    test("should wait for background sessionId and set metadata for TUI toolcall counting", async () => {
      ...
      const mockManager = {
        ...
        launch: async () => {
          const task = { id: "bg_1", status: "pending", description: "Test task", agent: "lookout" }
       ...
       const mockClient = {
         app: { agents: async () => ({ data: [{ name: "lookout", mode: "subagent" }] }) },
         config: { get: async () => ({}) },
         provider: { list: async () => ({ data: { connected: ["openai"] } }) },
         model: { list: async () => ({ data: [{ provider: "openai", id: "gpt-5.3-codex" }] }) },
         session: {
      ...
      const args = {
        description: "Lookout task",
        prompt: "Lookout features directory deeply",
  ...
  describe("category model resolution fallback", () => {
    test("category uses resolved.model when connectedProvidersCache is null and availableModels is empty", async () => {
      ...
      // NO userCategories override, NO cadetModel
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
        // userCategories: undefined - use DEFAULT_CATEGORIES only
        // cadetModel: undefined
    ...
    test("cadet model override takes precedence over category model", async () => {
      // given - cadet override model differs from category default
      const { createDelegateTask } = require("./tools")
      let launchInput: any
      ...
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
        cadetModel: "anthropic/claude-sonnet-4-6",
        connectedProvidersOverride: TEST_CONNECTED_PROVIDERS,
        availableModelsOverride: createTestAvailableModels(),
      })
    ...
    test("explicit category model takes precedence over cadet model", async () => {
      // given - explicit category model differs from cadet override
      const { createDelegateTask } = require("./tools")
      let launchInput: any
    ...
    test("cadet model override works with quick category (#1295)", async () => {
      // given - user configures agents.crew-junior.model but uses quick category
      const { createDelegateTask } = require("./tools")
      let launchInput: any
      ...
      // then - cadet override model should be used, not category default
      expect(launchInput.model.providerID).toBe("anthropic")
      expect(launchInput.model.modelID).toBe("claude-sonnet-4-6")
    })

    test("cadet model override works with user-defined category (#1295)", async () => {
      ...
      // then - cadet override model should be used as fallback
      expect(launchInput.model.providerID).toBe("openai")
  ...
  describe("buildSystemContent", () => {
    ...
    test("does not prepend plan agent prompt for strategist agent", () => {
      //#given - strategist is NOT a plan agent (decoupled)
      const { buildSystemContent } = require("./tools")
      ...
      //#then - strategist should NOT get plan agent system prepend
      expect(result).toBe(skillContent)
      expect(result).not.toContain("MANDATORY CONTEXT GATHERING PROTOCOL")
    })

    test("does not prepend plan agent prompt for Strategist (case insensitive)", () => {
      //#given - Strategist (capitalized) is NOT a plan agent
      const { buildSystemContent } = require("./tools")
      const skillContent = "You are a strategic planner"
    ...
    test("does not prepend plan agent prompt for non-plan agents", () => {
      ...
      // when
      const result = buildSystemContent({ skillContent, agentName: "sage" })
  ...
  describe("plan family mutual delegation block", () => {
    ...
    test("strategist cannot delegate to plan (cross-blocking)", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      const mockClient = {
         app: { agents: async () => ({ data: [{ name: "plan", mode: "subagent" }] }) },
      ...
      const result = await tool.execute(
        { description: "test", prompt: "Create a plan", subagent_type: "plan", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "strategist", abort: new AbortController().signal }
      )
    ...
    test("plan cannot delegate to strategist (cross-blocking)", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      const mockClient = {
         app: { agents: async () => ({ data: [{ name: "strategist", mode: "subagent" }] }) },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
         session: { get: async () => ({ data: { directory: "/project" } }), create: async () => ({ data: { id: "s" } }), prompt: async () => ({ data: {} }), promptAsync: async () => ({ data: {} }), messages: async () => ({ data: [] }), status: async () => ({ data: {} }) },
       }
       const tool = createDelegateTask({ manager: { launch: async () => ({}) }, client: mockClient })
      ...
      //#when
      const result = await tool.execute(
        { description: "test", prompt: "Execute", subagent_type: "strategist", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "plan", abort: new AbortController().signal }
      )
    ...
    test("captain CAN delegate to plan (not in plan family)", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      const mockClient = {
         app: { agents: async () => ({ data: [{ name: "plan", mode: "subagent" }] }) },
      ...
      const result = await tool.execute(
        { description: "test", prompt: "Create a plan", subagent_type: "plan", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "captain", abort: new AbortController().signal }
      )
  ...
  describe("subagent_type model extraction (issue #1225)", () => {
    test("background mode passes matched agent model to manager.launch", async () => {
      ...
      const mockManager = {
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "lookout", mode: "subagent", model: { providerID: "anthropic", modelID: "claude-haiku-4-5" } },
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
         session: {
           create: async () => ({ data: { id: "ses_lookout_model" } }),
           prompt: async () => ({ data: {} }),
           promptAsync: async () => ({ data: {} }),
           messages: async () => ({ data: [] }),
         },
      ...
      // when - delegating to lookout agent via subagent_type
      await tool.execute(
        {
          description: "Lookout codebase",
    ...
    test("sync mode passes matched agent model to session.prompt", async () => {
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "sage", mode: "subagent", model: { providerID: "anthropic", modelID: "claude-opus-4-6" } },
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
         session: {
           get: async () => ({ data: { directory: "/project" } }),
           create: async () => ({ data: { id: "ses_sage_model" } }),
           prompt: promptMock,
           promptAsync: promptMock,
           messages: async () => ({
             data: [{ info: { role: "assistant" }, parts: [{ type: "text", text: "Consultation done" }] }],
           }),
           status: async () => ({ data: { "ses_sage_model": { type: "idle" } } }),
         },
       }
      ...
      // when - delegating to sage agent via subagent_type in sync mode
      await tool.execute(
        {
          description: "Consult sage",
    ...
    test("agent without model resolves via fallback chain", async () => {
      ...
      await tool.execute(
        {
          description: "Lookout without model",
    ...
    test("agentOverrides model takes priority over matchedAgent.model (#1357)", async () => {
      // given - user configured sage to use a specific model in opencode-crew.json
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "sage", mode: "subagent", model: { providerID: "openai", modelID: "gpt-5.2" } },
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
       ...
       const tool = createDelegateTask({
         ...
         client: mockClient,
         agentOverrides: {
           sage: { model: "anthropic/claude-opus-4-6" },
         },
       })
      ...
      // when - delegating to sage via subagent_type with user override
      await tool.execute(
        {
          description: "Consult sage with override",
          prompt: "Review architecture",
          subagent_type: "sage",
          run_in_background: false,
          load_skills: [],
    ...
    test("agentOverrides variant is applied when model is overridden (#1357)", async () => {
      // given - user configured sage with model and variant
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const tool = createDelegateTask({
         ...
         client: mockClient,
         agentOverrides: {
           sage: { model: "anthropic/claude-opus-4-6", variant: "max" },
         },
       })
      ...
      // when - delegating to sage via subagent_type with variant override
      await tool.execute(
        {
          description: "Consult sage with variant",
          prompt: "Review architecture",
          subagent_type: "sage",
          run_in_background: false,
          load_skills: [],
    ...
    test("fallback chain resolves model when no override and no matchedAgent.model (#1357)", async () => {
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "sage", mode: "subagent" }, // no model field
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
      ...
      // when - delegating to sage with no override and no matchedAgent model
      await tool.execute(
        {
          description: "Consult sage with fallback",
          prompt: "Review architecture",
          subagent_type: "sage",
          run_in_background: false,
          load_skills: [],
      ...
      // then - should resolve via AGENT_MODEL_REQUIREMENTS fallback chain for sage
      // sage fallback chain: gpt-5.2 (openai) > gemini-3.1-pro (google) > claude-opus-4-6 (anthropic)
  ...
  describe("subagent task permission", () => {
    test("plan subagent should have task permission enabled", async () => {
      //#given - captain delegates to plan agent
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      ...
      //#when - captain delegates to plan
      await tool.execute(
        {
    ...
    test("strategist subagent should have task permission (plan family)", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      const promptMock = async (input: any) => { promptBody = input.body; return { data: {} } }
       const mockClient = {
         ...
         session: {
           get: async () => ({ data: { directory: "/project" } }),
           create: async () => ({ data: { id: "ses_strategist_task" } }),
           prompt: promptMock,
           promptAsync: promptMock,
           messages: async () => ({ data: [{ info: { role: "assistant" }, parts: [{ type: "text", text: "Plan created" }] }] }),
           status: async () => ({ data: { "ses_strategist_task": { type: "idle" } } }),
         },
       }
       const tool = createDelegateTask({ manager: { launch: async () => ({}) }, client: mockClient })
      ...
      //#when
      await tool.execute(
        { description: "Test strategist task permission", prompt: "Create a plan", subagent_type: "strategist", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "captain", abort: new AbortController().signal }
      )
    ...
    test("non-plan subagent should NOT have task permission", async () => {
      //#given - captain delegates to sage (non-plan)
      const { createDelegateTask } = require("./tools")
      let promptBody: any
      
      const mockManager = { launch: async () => ({}) }
      const mockClient = {
        app: { agents: async () => ({ data: [{ name: "sage", mode: "subagent" }] }) },
        config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
        session: {
          get: async () => ({ data: { directory: "/project" } }),
          create: async () => ({ data: { id: "ses_sage_no_delegate" } }),
          prompt: async (input: any) => {
            promptBody = input.body
            return { data: {} }
          },
          ...
          status: async () => ({ data: { "ses_sage_no_delegate": { type: "idle" } } }),
        },
      }
      ...
      // when - captain delegates to sage
      await tool.execute(
        {
          description: "Test sage no task permission",
      ...
      // then - sage should NOT have task permission
  ...
  describe("session title and metadata format (OpenCode compatibility)", () => {
    test("sync session title follows OpenCode format: '{description} (@{agent} subagent)'", async () => {
      ...
      expect(createBody.title).toBe("Implement feature X (@Cadet subagent)")
    }, { timeout: 10000 })
