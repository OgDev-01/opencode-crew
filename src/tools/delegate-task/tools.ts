...
import { CADET_AGENT } from "./cadet-agent"
...
export function createDelegateTask(options: DelegateTaskToolOptions): ToolDefinition {
  ...
  **CORRECT - Using subagent_type:**
  \`\`\`
  task(subagent_type="lookout", load_skills=[], description="Find patterns", prompt="...", run_in_background=true)
  \`\`\`
  
  REQUIRED: Provide ONE of:
  - category: For task delegation (uses Cadet with category-optimized model)
  - subagent_type: For direct agent invocation (lookout, archivist, sage, etc.)
  
  **DO NOT provide both.** If category is provided, subagent_type is ignored.
  
  - load_skills: ALWAYS REQUIRED. Pass [] if no skills needed, or ["skill-1", "skill-2"] for category tasks.
  - category: Use predefined category → Spawns Cadet with category config
    Available categories:
  ${categoryList}
  - subagent_type: Use specific agent directly (lookout, archivist, sage, assessor, critic)
  ...
  **WHEN TO USE session_id:**
  ...
  return tool({
    ...
    async execute(args: DelegateTaskArgs, toolContext) {
      ...
      if (args.category) {
        if (args.subagent_type && args.subagent_type !== CADET_AGENT) {
          log("[task] category provided - overriding subagent_type to cadet", {
            category: args.category,
            subagent_type: args.subagent_type,
          })
        }
        args.subagent_type = CADET_AGENT
      }
      await ctx.metadata?.({
      ...
      let fallbackChain: import("../../shared/model/model-requirements").FallbackEntry[] | undefined
