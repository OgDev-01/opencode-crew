...
describe("loadJsonFile", () => {
  ...
  it("discovers JSONC-only user config (opencode-crew.jsonc)", () => {
    const originalEnv = process.env.OPENCODE_CONFIG_DIR
    const tempBase = join(tmpdir(), `crew-test-user-jsonc-${Date.now()}-${Math.random().toString(36).slice(2)}`)
    try {
  ...
  "lsp": {
    "user-jsonc": {
      ...
      const userPath = join(tempBase, "opencode-crew.jsonc")
  ...
  it("discovers JSONC-only opencode config (opencode.jsonc)", () => {
    const originalEnv = process.env.OPENCODE_CONFIG_DIR
    const tempBase = join(tmpdir(), `crew-test-oc-jsonc-${Date.now()}-${Math.random().toString(36).slice(2)}`)
    try {
      mkdirSync(tempBase, { recursive: true })
      process.env.OPENCODE_CONFIG_DIR = tempBase
  ...
  it("discovers JSONC-only project config (.opencode/opencode-crew.jsonc)", () => {
    const originalCwd = process.cwd()
    const tempProject = join(tmpdir(), `crew-test-project-jsonc-${Date.now()}-${Math.random().toString(36).slice(2)}`)
    try {
  ...
  "lsp": {
    "project-jsonc": {
      ...
      const projectPath = join(tempProject, ".opencode", "opencode-crew.jsonc")
  ...
  it("prefers .jsonc over .json when both exist for same config id", () => {
    const originalEnv = process.env.OPENCODE_CONFIG_DIR
    const tempBase = join(tmpdir(), `crew-test-precedence-${Date.now()}-${Math.random().toString(36).slice(2)}`)
    try {
      mkdirSync(tempBase, { recursive: true })
      process.env.OPENCODE_CONFIG_DIR = tempBase
  ...
  "lsp": {
    "conflict": {
      ...
      writeFileSync(join(tempBase, "opencode-crew.json"), jsonContent, "utf-8")
      writeFileSync(join(tempBase, "opencode-crew.jsonc"), jsoncContent, "utf-8")
