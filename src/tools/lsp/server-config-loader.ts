...
import { parseJsonc, detectConfigFile } from "@/shared/config/jsonc-parser"
...
export function getConfigPaths(): { project: string; user: string; opencode: string } {
  ...
  return {
    project: detectConfigFile(join(cwd, ".opencode", "opencode-crew")).path,
    user: detectConfigFile(join(configDir, "opencode-crew")).path,
