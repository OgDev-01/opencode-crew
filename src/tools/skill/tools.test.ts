...
describe("skill tool - agent restriction", () => {
  ...
  it("allows skill when agent matches restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("restricted-skill", { agent: "captain" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "captain" }
  ...
  it("throws error when agent does not match restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("captain-only-skill", { agent: "captain" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "sage" }

    // when / #then
    await expect(tool.execute({ name: "captain-only-skill" }, context)).rejects.toThrow(
      'Skill "captain-only-skill" is restricted to agent "captain"'
    )
  })
  ...
  it("throws error when context agent is undefined for restricted skill", async () => {
    ...
    // when / #then
    await expect(tool.execute({ name: "captain-only-skill" }, contextWithoutAgent)).rejects.toThrow(
      'Skill "captain-only-skill" is restricted to agent "captain"'
    )
  })
