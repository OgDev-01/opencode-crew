...
describe("createTaskList", () => {
  ...
  beforeEach(() => {
    taskDir = join(testProjectDir, ".crew/tasks")
  ...
  it("returns empty array when no tasks exist", async () => {
    ...
    const config = {
      captain: {
        tasks: {
          storage_path: join(testProjectDir, ".crew/tasks"),
  ...
  it("excludes completed tasks by default", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, ".crew/tasks", "T-1.json"), task1)
    writeJsonAtomic(join(testProjectDir, ".crew/tasks", "T-2.json"), task2)
  ...
  it("excludes deleted tasks by default", async () => {
   ...
   it("returns summary format with id, subject, status, owner, blockedBy", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, ".crew/tasks", "T-1.json"), task)
    ...
    expect(summary.owner).toBe("captain")
  ...
  it("filters blockedBy to only include unresolved (non-completed) blockers", async () => {
    ...
    writeJsonAtomic(join(testProjectDir, ".crew/tasks", "T-blocker-completed.json"), blockerCompleted)
    writeJsonAtomic(join(testProjectDir, ".crew/tasks", "T-blocker-pending.json"), blockerPending)
    writeJsonAtomic(join(testProjectDir, ".crew/tasks", "T-main.json"), mainTask)
